...
| 2026-09-15 | §9 | **Phase 1 Stage 2 — the results logframe on the dashboard.** `domain/logframe.js` composes one row per IRMF indicator — baseline and target off the Stage 1 `results` map, the current figure off the record's own indicator field — reading paths and computing nothing; direct and indirect beneficiaries stay separate rows and an untouched indicator is absent rather than zeroed. The readiness route returns it beside the six-criteria scorecard, and the project page renders it as a baseline→current→target→year table. `tests/gcf-logframe.test.js` proves the composer and the route on both stores | this repository |
| 2026-09-15 | §9 | **Phase 1 Stage 7 — the demo close-out (Phase 1 complete).** `tests/gcf-assessment-journey.test.js` walks the whole assessor flow through the store on both backends — record → validate through the lifecycle → assessment report carries the sign-off → return snapshots the gaps → resubmit → the comparison shows the gap resolved — following one distinctive rating everywhere it is read, so a re-keyed value would show. Six conformance rules (G-VAL-01/02/03, G-RPT-01, G-RET-01/02) map the validation lifecycle, the report and the return loop to their proving tests, added to `domain/conformance.js`, regenerated into `docs/GCF-CONFORMANCE.md` (47 rules) and proved by execution in `docs/CONFORMANCE-EVIDENCE.md` (153 executable rules). `e2e/gcf-pipeline.spec.js` drives the assessor panel in Chromium — start review, rate, validate with conditions, download the report, return to sponsor, download the letter — and asserts the page never widens at a phone width. With this the DFCC GCF assessment product (Phase 0 + Phase 1's seven stages) is complete | this repository |
| 2026-09-15 | §9 | **Phase 1 Stage 6 — the return-to-sponsor loop.** `domain/return-loop.js` derives a gap list from the assessment (sub-criteria the record does not hold + criteria the assessor rated weak), each with a remedy, and compares today's gaps against the last recorded return into resolved / still-outstanding / newly-raised. A return is a recorded event so the comparison has a fixed point: `POST /v1/gcf/pipeline/:id/return` (validate scope) snapshots the gaps into `validation.returns[]`, refused with 409 NOTHING_TO_RETURN unless validated with a non-clean recommendation, and refuses the sample. `application/return-letter.js` renders the sponsor's letter (decision, gaps by criterion with remedies, what next) through the shared report standard — a working document, not a GCF decision. `GET /v1/gcf/pipeline/:id/return` (read) serves gaps + comparison + returns; `GET /v1/gcf/pipeline/:id/return-letter?format=json\|pdf\|word` (read) the letter. The assessor panel shows the gap list, a validate-gated return control, the letter download and the comparison. `tests/gcf-return-loop.test.js` proves the gaps, the comparison, the refusals, the scope and a well-formed letter PDF | this repository |
