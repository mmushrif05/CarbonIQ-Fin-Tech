...
| GCF pipeline | `G-DATA-04` | ToR Lot 1, Milestone 4 — durable capture | `tests/gcf-pipeline.test.js` | 43 | executed |
...
| GCF pipeline | `G-CYCLE-02` | ToR Lot 1, Milestone 4 — systems and procedures to capture data; a pipeline has a time axis | `tests/gcf-cycle.test.js` | 83 | executed |
...
| GCF pipeline | `G-CYCLE-08` | ToR Lot 1, Milestone 4 — illustrative data replaced by the entity’s own, never mixed with it | `tests/gcf-cycle.test.js` | 83 | executed |
| GCF pipeline | `G-ACCR-05` | Board decision B.36/10 — DFCC’s accreditation envelope; the entity’s own where recorded | `tests/gcf-cycle.test.js` | 83 | executed |
