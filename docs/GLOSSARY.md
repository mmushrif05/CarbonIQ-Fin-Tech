...
| **access window** | `accessEndsAt` on an account: the instant after which it may no longer sign in. This is what a **trial** is. Not a role (which says what an account may do) and not the `active` flag (which is a decision an administrator took, rather than a date that passed) — the three are separate because a customer told the wrong one calls the wrong person. |
| **standing** | whether an administrator has switched an account off. Refused as `ACCOUNT_DISABLED`, never as `ACCESS_ENDED`. |
| **issued password** | one an administrator typed. The account carries `mustChangePassword` and reaches its own password and nothing else until it is replaced. |
| **the bootstrap window** | the state in which `POST /v1/auth/bootstrap` will create the first administrator: no accounts at all on the deployment, `ADMIN_BOOTSTRAP_TOKEN` set, and a writable store. It closes with the account it creates and cannot be reopened. |
