...
| Portfolio | §5.2 roll-up over a **persisted** register (migration 0008): one row per exposure per reporting year holding the input and the result, a stored projection the roll-up reads instead of the trace (316 ms over 10,000), real coverage against the entity's stated book total, and the improvement plan | The same register reaching §5.1 and §5.3; the lock-and-supersede lifecycle; economic intensity across a book; recalculation policy and threshold on entity settings |
...
| Surface | The stateless engine routes (`read`), and the register: `exposures` CRUD and `recompute`, `book`, `years`, `position/:year`, `storage` (the writes on `write`, deliberately apart from the engine's `read`) | A reporting-year screen; a §5.2 screen |
