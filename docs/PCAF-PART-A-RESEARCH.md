...
**CarbonIQ status: built (2026-09-16), on the register.** **Sri Lankan relevance: moderate-to-high** —
...
| Reporting | The §5.2 annual disclosure and the per-exposure report — one content model in Chapter 6 order, rendered to PDF and Word by the platform report standard (the renderer Part C shares); coverage, the outstanding-weighted score with scope 3 apart, the factor set named, Annex 10.2 by sector, the improvement plan as the limitations section; the twelve Chapter 6 requirements answered from the report; `GET /v1/pcaf/part-a/disclosure/:year`, `POST /v1/pcaf/part-a/exposures/:id/report`, JSON/PDF/Word | The report for the other asset classes; a generated conformance-evidence run (the matrix and its resolving test exist) |
| Conformance | §5.2 report: rule → implementation → proving test (`reporting/conformance.js`), held by a resolving-citation test | The generated markdown document and the coverage-execution evidence run; the engine's own conformance matrix |
...
| Surface | The stateless engine routes (`read`), and the register: `exposures` CRUD and `recompute`, `book`, `years`, `position/:year`, `storage` (the writes on `write`, deliberately apart from the engine's `read`); the **Lending Book** screen over the register (`ui/pages/parta-register.html`), computing nothing, with a sample book for a preview session; the **§5.2 disclosure and per-exposure report** (`disclosure/:year`, `exposures/:id/report`, JSON/PDF/Word) | A reporting-year screen across classes; the report for the other classes |
...
| ~~8~~ **done** | **The Part A report and the DCL** — shipped for §5.2; see §2.2, `docs/PCAF-PART-A-BUSINESS-LOANS.md` §9 | The deliverable a bank actually files | One content model in the DCL's order, two renderers via Part C's report standard; twelve requirements answered from the report; conformance matrix and generated document | Every DCL item answers from a rendered fact; `npm run docs:conformance` covers Part A |
...
|---|---|---|---|
| 2026-09-16 | §2.6, §11 | §5.6 motor vehicle loans built: Table 5.6-1 held in its own file (1a = 1b = 1; 2a 2; 2b 3; 3a 4; 3b 5); option derived per vehicle; fn 146 local = Sri-Lanka-wide, so certificate efficiency × the registry's ESMAP 2000 distance is 2a; lowest quality in the mix per borrower (p.93); 100 % attribution where the origination value is unknown (p.91); hybrid and plug-in rules (p.96); production emissions a first-year scope 3 lump sum. Class-average efficiencies are a provisional table, not evidence. | Part A Third Edition pp.90–96, Table 5.6-1 (p.94), Annex Table 10.1-6 (p.196) |
| 2026-09-16 | §5.1–§5.5, Chapter 6 | One exposure register across the built classes: each class registers its engine, its preparation and an adapter onto the register's one shape (`application/register-classes.js`); the position is rolled up per class and never across classes (DCL p.128 — one outstanding-weighted score per class); the consolidated disclosure names one dataset release per recorded class. | Part A Third Edition Tables 5.1-2, 5.2-1, 5.3-1, 5.4-1/5.5-1; DCL p.128 |
| 2026-09-16 | §5.4 p.79 / §5.5 | Floor area with its unit: converted once in the engine at 1 ft² = 0.09290304 m² (exact, 1 ft = 0.3048 m), the area as keyed carried on the trace; a land unit (perch, acre, hectare) refused because the intensity is per square metre of floor. | §5.4 p.79 — energy by source × factor, statistics per m² of floor area |
| 2026-09-16 | §2.3, §2.4, §2.9, §10 | **Baseline research (Part 1 of the Amana readiness plan), recorded in `docs/BASELINE-REGISTER.md` before the code that acts on it.** Three corrections to what this repository held: (i) `data/pcaf-parta/country-config.json` labelled the 2017 *build* margin (0.9224 tCO2e/MWh) as Sri Lanka's operating margin — the 2017 OM is 0.6993, and the file now carries SLSEA's 2022 set (OM 0.7123 · BM 0.5841 · CM 0.6482, `secondary_reported`) with Ember's 2024 generation-weighted average 0.3757 (`primary`, read from the file) in place of the LTGEP's rounded 0.5; (ii) the sovereign dataset's Sri Lanka row (35 Mt excl. / 30 Mt incl. LULUCF, "illustrative") had the LULUCF term the wrong way — NC3 2010 reports 22,084.62 Gg excl. and a net land-sector **sink** of 18,366 Gg, so the including-LULUCF figure is 3,718.45 Gg, not 30 Mt; (iii) the provisional building intensities (office 160, retail 210 kWh/m²) sit below every Sri Lankan figure found (office ~250, supermarket 465.4). Two findings that shape the classes still to build: Sri Lanka is a **named country** in Open CEDA 2025 (CC BY-SA 4.0), so §5.2 Option 3 factors need not come from a rest-of-world bucket; and under §5.6 fn 146 a Sri-Lanka-wide distance statistic is *local*, so a vehicle facility reaches Option 2a (score 2) from the registration certificate alone. Every external figure is `secondary_reported` until re-read: the session's network policy refused every government and vendor host | `docs/BASELINE-REGISTER.md` §3–§5; PCAF Third Edition pp.79, 85, 93–94, 141–147, 162, 167 |
| 2026-09-16 | §2.4, §10 | **The §5.4/§5.5 engine resolves its factors from the baseline registry.** `application/property-factors.js` resolves the grid average, the fuel factors and the building-type intensities in force for the country and organisation and hands them in; a resolved figure replaces the provisional table's row and the trace names the scope and version; a figure the registry does not hold falls back to the table for that one figure and says so | `tests/baseline-wiring.test.js`; PCAF §5.4 p.79, §5.5 p.85 |
| 2026-09-14 | §10, §11 | **The screens over the position.** The consolidated route gained its screen (Financed Emissions): the outstanding Chapter 6 items with their clauses, the headline with its boundaries named and scope 3 apart, every class as a row with its reason, one score per class, the entity’s facts recorded from the screen, and the disclosure and register as downloads. The two books gained their disclosures and per-exposure reports as downloads, finding codes in words, and an empty year told apart from a failure. The reference names the route that prices each class (`assessRoute`), so the engine form no longer offers a class that answers 501. The screens compute nothing; a test refuses a sum, a division or an average in the module | CarbonIQ; `ui/js/parta-position.js`, `tests/parta-position-ui.test.js`, `e2e/parta-position.spec.js` |
| 2026-09-14 | §4, §10, §11 | **The document a verifier can sign, and the one document a bank files.** A review of the §5.2 and §5.9 disclosures from the seat of an ISO 14064-3 / ISAE 3000 verifier found the arithmetic assurable and the apparatus around it not: no organisational boundary or consolidation approach (and a checklist item claiming one that always answered Yes), no per-exposure audit-trail annex, no responsible party, no uncertainty statement, no document identity, the verifier never named on a "verified" cover, seven checklist items that could never fail, scope 1 and 2 never shown apart, no portfolio intensity, a cover reading *Re/insurer* on a bank's document, a literal `undefined` in the methodology line, and — structurally — no consolidated Part A disclosure at all, only four per-class documents each saying it was "one input to a Chapter 6 disclosure". Every item is closed. **The reporting entity is a recorded fact, not a query string**: legal name, consolidation approach (operational control · financial control · equity share), boundary note, fiscal year-end, GWP basis, who prepared and who approved, and the asset classes not reported with their reasons (Chapter 6, p.162) live on the org-wide Part A settings, null by default, and every document prints each as stated or "Not stated" — never a default. **The cover names the reporting entity, the responsible party and the document's identity** — a reference derived from a SHA-256 over the canonical facts (so one position rendered twice is one reference and a changed figure is another), the build commit, and the content hash; the shared renderer takes the label from the model, so Part C's documents are byte-identical. **New sections in every Part A document**: reporting entity and boundary (first), scope 1 and scope 2 apart beside the combined line, the option distribution beneath the weighted score, an economic intensity the roll-up computes (DCL p.127) across the class and per sector, the improvement steps with their scenario scores, the footnote 71 fluctuation analysis answered from the book, an uncertainty statement by score in PCAF's own terms with everything the document does not contain in one list, the exposure register as an audit-trail annex (one row per exposure, each resolving to its stored record), every row of the factor set with the whole checksum, and where each SLFRS S2 §29(a)(vi) / B58–B63 line and CBSL Direction No. 05 of 2022 comes from. **The checklist can fail**: no item is a constant, GOV-1/COV-2/REC-1/FLU-1/INT-1 read the facts, and GOV-2, PER-1, TRC-1, UNC-1, DOC-1 and ASR-2 are new; INV-1 stays No by design. The per-exposure and per-holding reports now receive the entity's settings, so two documents from one book can no longer contradict each other on the recalculation protocol. **The consolidated disclosure** (`GET /v1/pcaf/part-a/financed-emissions/:year`, `/disclosure` as JSON, PDF or Word, `/register.csv`) lays every asset class side by side: the headline sums each recorded class on the boundary its section reports (§5.2 scope 1 and 2; §5.9 scope 1 excluding LULUCF) and names them, scope 3 apart; one data-quality score per class and never averaged across tables; coverage summed only across the classes in the book's currency with the class it excludes named — which surfaced that the sovereign register, denominated in international USD by construction, was already dividing its outstanding by an LKR book total; every class not recorded is a row with a reason, the entity's stated reason overriding the system's and never the reverse. Sixteen conformance rules (A-DOC-01…16) prove all of it by execution; 138 executable rules across the three matrices now prove | ISO 14064-3 §§5.2, 6.1.3, 6.1.5, 9; ISAE 3000 §§12, 48, 69; Part A ch.6 (pp.160–169); DCL pp.124, 127, 128; SLFRS S2 §29(a)(vi), B58–B63 |
| 2026-09-12 | §2.4, §2.5, §11 | **§5.4 CRE + §5.5 mortgages, step RE2 — the shared real-estate engine.** One engine (`src/domains/pcaf-part-a/domain/real-estate/`) computes the one calculation both classes share: financed building emissions = building scope 1 and 2 × (outstanding ÷ property value at origination). `classify.js` is the gate — it names what is *not* this class rather than assess it: a listed CRE owner → §5.1 (its building's emissions are already in the corporate inventory the listed value carries), a property-secured loan for another purpose → §5.2, and for mortgages a HEL/HELOC or a construction/renovation loan is out of scope (the homeowner does not account for the builder's emissions, fn 132); CRE construction/renovation is **optional not excluded**, so it runs with a note. `denominator.js` fixes the origination value with three states (`origination` · `latest-fixed` · `modified`) and refuses an attribution factor above 1. `energy.js` earns the option from **how the energy is known** — metered × supplier factor (1a) or × average factor (1b), an official label × floor area (2a), building-type statistics × floor area (2b), statistics × building count (3) — and returns the building's own scope 1 (on-site fuel) and scope 2 (electricity), combined at minimum with the split carried, before any attribution. `index.js` orchestrates and attributes; a developer-reported construction figure for CRE is carried as **scope 3 category 15, separate and never summed** with operational scope 1 and 2, and is absent-not-zero by default. The data-quality score reads the RE1 table by (class, option). `POST /v1/pcaf/part-a/real-estate/assess` is stateless (`read`) and stores nothing. The standard's shape is proved: an office at 1,000 m² on the provisional LK set yields building scope 2 ≈ 72.08, scope 1 ≈ 6.41, combined ≈ 78.49 tCO2e, and at a 0.25 attribution factor financed ≈ 19.62 tCO2e. Step RE2 of the §5.4/§5.5 plan | §5.4 (pp.77–81); §5.5 (pp.83–88); the §5.2/§5.9 engine pattern (classify → denominator → energy/option → attribute) |
| 2026-09-12 | §2.4, §2.5, §11 | **§5.4 CRE + §5.5 mortgages, step RE1 — the shared data library, the property data-quality table and the reference.** CRE and mortgages share one denominator rule (property value at origination, fixed, updated only on a modification), one energy equation and **one data-quality table** (Tables 5.4-1 / 5.5-1), so the table is held once (`data/pcaf-parta/dq-real-estate.json`) and both class keys resolve to it. Its numbers are its own and never the corporate classes': **Option 2a is score 3 here where §5.1/§5.2 put it at 2, and 2b is 4 where they put it at 3** (p.80, p.86), and there is no 3a/3b — Option 3 is a single row at score 5. `data/pcaf-parta/real-estate/energy-statistics.json` holds the estimation input the engine reaches for when energy is not metered: energy-use intensity (kWh/m²/yr) by building type for Option 2b, a typical floor area per building for Option 3, illustrative label-class intensities for Option 2a (Sri Lanka has no label scheme, fn 129), and provisional grid and fuel emission factors — **every figure provisional and says so**, versioned/dated/checksummed (`MANIFEST.json`, `npm run docs:parta-realestate-manifest`, wired into the CI docs job) like the sovereign set. `GET /v1/pcaf/part-a/reference` now carries `commercial-real-estate` and `mortgages` with the shared table, the origination-value denominator and the building-type vocabulary. Step RE1 of the §5.4/§5.5 plan | §5.4 (pp.77–81); §5.5 (pp.83–88); Tables 5.4-1/5.5-1 (p.80, p.86); the §5.9 data-library pattern |
| 2026-09-11 | §2.9, §4, §10, §11 | **§5.9 Sovereign debt, step 6 — the conformance matrix, and the close-out.** The §5.9 engine, checks, register and report now carry their own conformance evidence, in the shape §5.2, Part C and the GCF pipeline already use: sixteen rules added to the Part A matrix (`src/domains/pcaf-part-a/domain/conformance-rules.js`) mapping each §5.9 rule — the PPP-GDP attribution and the worked example (106/91), scope 1 on both LULUCF boundaries never summed, scope 2/3 as absent-not-zero shoulds, Table 5.9-6 data quality and its outstanding-amount weighting, whole-book coverage, the four data-truth checks, the register (one bond once, both halves kept, the stored projection, the empty-year 409), the disclosure and the checklist that can never reach 100% — to the code that enforces it and the test that proves it. It is **one Part A matrix**, not two: sovereign debt is a second built asset class on it, kept apart from Part C's because the three scopes never merge and the option-to-score numerals are not interchangeable. `GET /v1/pcaf/part-a/conformance` serves all fifty-nine rules; `docs/PCAF-PART-A-CONFORMANCE.md` is regenerated from it; `tests/pcaf-parta-conformance.test.js` fails on a citation that does not resolve; and `scripts/conformance-evidence.js` re-proves each by execution — **all 122 executable rules across the three matrices proved by their own test**. Step 6 of the §5.9 plan; §5.9 is built end to end | the north star's "every module carries its own conformance evidence"; the §5.2 conformance matrix |
| 2026-09-11 | §2.9, §4, §10, §11 | **§5.9 Sovereign debt, step 5 — the disclosure report and the Disclosure Checklist.** The document PCAF Chapter 6 asks a bank to file for its sovereign book, built from the register's reporting-year position in one content model in the checklist's order and rendered to PDF and Word by the one platform renderer §5.2 and Part C already share (`src/domains/pcaf-part-a/reporting/sovereign/`). Its shape is the §5.9 shape, not §5.2's: scope 1 on **both LULUCF boundaries, never summed** (the including-LULUCF line partial with a held-count where some sovereigns hold no including figure), scope 2 and 3 as absent-not-zero shoulds, the PPP-GDP methodology, one data-quality score weighted by **outstanding amount** (p.128, never premium), coverage over the shared whole book (p.124), a by-sovereign Annex 10.2, the data-truth checks as the limitations section, and PCAF's own recorded limitations for the class (the scope analogy, the accepted double count, PPP-GDP a flow). `GET /v1/pcaf/part-a/sovereign/disclosure/:year` and `POST /v1/pcaf/part-a/sovereign/exposures/:id/report` serve JSON, PDF or Word, both `read` and storing nothing; a year with no sovereign holdings is a **409**. The completed checklist covers the §5.9 asset class only, so it **cannot reach a hundred per cent** — the entity-inventory item is answered No with the reason, the same rule §5.2 and the GCF report follow, and the distinctive §5.9 item (scope 1 on both LULUCF boundaries) is answered No and honest where the including figure is not held rather than fabricating an equality. `tests/parta-sovereign-report-golden.test.js` pins the disclosure and the per-holding report to committed goldens (SG 106 + HK 91 + LK proxy = 196.9 tCO2e excl-LULUCF, weighted score 3.5). Step 5 of the §5.9 plan | Chapter 6 (pp.160–174); Annex 10.2 (pp.199–200); §5.9 (p.141); DCL pp.124, 128; the §5.2 report; the north star's "one renderer" |
| 2026-09-11 | §2.9, §10, §11 | **§5.9 Sovereign debt, step 4b — the Sovereign Book screen.** `ui/pages/parta-sovereign.html` + `ui/js/parta-sovereign.js` + `ui/css/parta-sovereign.css`, the §5.9 mirror of the Lending Book, over the sovereign register API and **computing nothing** — every figure is one the register returned, and `tests/parta-sovereign-ui.test.js` holds it there. It reads in the order a bank reads its own book: the position, with financed scope 1 shown on **both LULUCF boundaries and never summed** (the including line partial, with a held-count) and scope 3 a separate line; the shared book total, so coverage is a percentage without leaving the screen; the sovereigns that carry the position; the improvement plan, the material findings first; and each holding opening into its lines, its attribution (the PPP-GDP equation the engine ran, no division done here), its production intensity and its findings inline with what clears them. Recording is a **one-field intake** — a held country needs only an amount — with a custom path for a country the dataset does not hold; the engine runs before anything is written and a refusal reaches the screen with its clause. Every write control is hidden for a preview session (the server's `read` scope being the control), and a preview visitor sees a three-holding sample book (Singapore, Hong Kong, Sri Lanka) installed by the same installer that seeds §5.2's, chosen so the emissions-lag, one-sided-LULUCF and proxy-country findings are all on screen. Wired into the sidebar, the router, the role gate (60, the PCAF bar) and `PREVIEW_PAGES`. `e2e/parta-sovereign.spec.js` seeds a book through the API and drives all of it, then asserts the page never widens at 430px. Step 4b of the §5.9 plan | `docs/PCAF-PART-A-BUSINESS-LOANS.md` §8 (the Lending Book pattern); §5.9 (p.141); DCL pp.124, 128 |
| 2026-09-11 | §2.9, §10, §11 | **§5.9 Sovereign debt, step 4 — the persisted register.** S2 shipped the engine as a stateless read; a sovereign holding had to be posted whole on every call, so coverage could not be stated and a figure could not be traced. `migration 0011` adds `parta_sovereign_exposures` with a stored roll-up projection (the same second-versus-forty-milliseconds discipline as `parta_exposures`), and it is a **separate table** from the §5.2 register — not a discriminator on it — because the two result shapes differ (scope 1 on two LULUCF boundaries and one data-quality score, against six lines and two scores), so one generated column cannot compute both. `parta_book` is **shared**: coverage is against the same whole-book denominator and the recalculation protocol is one entity-wide claim, neither class-specific. `application/sovereign-register.js` records, reads, updates, removes and recomputes, following every rule the §5.2 register set — both halves kept, nothing recomputes on read, one bond once (`409 DUPLICATE_SOVEREIGN_HOLDING` + a partial unique index on `(org, year, account_number)`), a `409` on an empty year. `domain/sovereign/portfolio.js` rolls up the position: scope 1 summed on each LULUCF boundary and **never added together** (the including sum is partial and says so with a held-count), scope 3 apart, the disclosed score weighted by **outstanding amount** (p.128, the §5.2 weighting, never Part C's premium), coverage as assessed outstanding over the stated book (DCL p.124) or absent, a per-sovereign breakdown and the findings grouped as an improvement plan. Routes under `/v1/pcaf/part-a/sovereign/exposures` (CRUD + recompute, scope `write`) and `/sovereign/position/:year`, `/sovereign/years` (`read`); a recompute reports movement across scope 1 both ways, scope 2/3, the score and the findings. The projected roll-up equals the whole-record roll-up figure for figure on both stores. **The Lending Book UI for sovereign rows is S4b, still to come.** **Operator action: production needs `npm run db:migrate` for 0011 before the register routes work; the engine and assess route are unaffected.** Step 4 of the §5.9 plan | §5.9 pp.140–148; DCL p.124; Part A p.128; the §5.2 register (migrations 0008/0009) |
| 2026-09-11 | §2.9, §9, §10, §11 | **§5.9 Sovereign debt, step 3 — the data-truth checks, the third verdict applied to a sovereign.** §5.9 gives the method and says almost nothing about whether the country data is telling the truth; `src/domains/pcaf-part-a/domain/sovereign/checks.js` fills that gap the way §5.2 does — findings that refuse nothing and change no figure, each carrying what would clear it, on the `validation` block of the assess response. Six checks. **`SOVEREIGN_PROXY_COUNTRY`** (material) fires from the resolved Option 3b: the figure rests on another country's inventory. **`SOVEREIGN_EMISSIONS_LAG`** (material) fires where the emissions year is at or beyond a threshold behind the reporting year — Singapore's EDGAR 2018 figure against a 2024 report raises it; PCAF permits the lag (Chapter 4, p.31) and EDGAR's own series runs four years behind (Table 10.3-4), so the threshold is CarbonIQ's. **`SOVEREIGN_LULUCF_ONE_SIDED`** (material) fires where scope 1 is held on one LULUCF boundary only, which §5.9 (p.141) asks to be reported both ways. **`SOVEREIGN_INTENSITY_IMPLAUSIBLE`** (material) is the guardrail against the distortion PPP-GDP exists to remove: a production intensity outside a plausible band (default 10–2000 tCO₂e per million international USD, CarbonIQ's) almost always means the denominator or the emissions are in the wrong units — a debt figure or GDP in billions reading as the 1,369× swing of Annex 10.3. **`SOVEREIGN_EMISSIONS_GDP_YEAR_GAP`** (advisory) fires where numerator and denominator come from years far apart. And the **independent-path check** follows the GCF rule exactly: where a second source is supplied under `crossCheck` it recomputes and reports the divergence (`SOVEREIGN_SOURCE_DIVERGENCE`, material), and where none is it says the cross-check could not run (`SOVEREIGN_NO_INDEPENDENT_SOURCE`, advisory) rather than passing silently. Four thresholds are CarbonIQ's rather than PCAF's, each named as ours on the finding it raises and settable per request (and surfaced on `GET /reference`). Step 3 of the §5.9 plan | §5.9 pp.140–148; Table 5.9-1 (p.141); Table 5.9-6 (p.147); Annex 10.3 (pp.201–204); Table 10.3-4 (pp.205–206); the §5.2 third verdict |
| 2026-09-11 | §2.9, §10, §11 | **§5.9 Sovereign debt, step 2 — the engine.** `src/domains/pcaf-part-a/domain/sovereign/` assesses one sovereign exposure and reproduces the standard's worked example exactly: `assessSovereign({ country: 'SG', exposure: { amount: 1e6 } })` → 106 tCO₂e, Hong Kong → 91 (Table 10.3-2, p.202). Four rules a general tool gets wrong are enforced. **The denominator is PPP-adjusted GDP, never equity plus debt** (`attribution.js`, p.144); there is no cap, and a factor above 1 is refused as an input error because a single institution's holding cannot exceed a nation's output. **Scope 1 is returned both including and excluding LULUCF and never summed** (p.141) — the excluding-LULUCF line is the headline per the standard's own example, and where no including-LULUCF figure is held (Singapore, Hong Kong) it is reported absent, not assumed equal. **Scope 2 (imported grid energy) and scope 3 (non-energy imports) are shoulds, reported absent rather than zero** where not held, scope 3 apart from scope 1+2. **The consumption view** (production − exported + imported, p.142) is recorded with its equation and limitations but not computed for want of trade-embodied data, and never folded into the territorial figure. Data quality is scored from Table 5.9-6 alone: the source basis maps to the option (`options.js` — UNFCCC verified→1a, unverified/EDGAR/Climate Watch→1b, energy-activity→2, sectoral→3a, proxy→3b), a caller may override with a justification, and a provisional figure earns no option on its own (`DQ_OPTION_REQUIRED`). `POST /v1/pcaf/part-a/sovereign/assess` (scope `read`, stateless), a country from the dataset or figures on the request. Step 2 of the §5.9 plan | §5.9 pp.140–148; Table 10.3-2 (p.202); Table 5.9-6 (p.147) |
| 2026-09-11 | §2.9, §10, §11 | **§5.9 Sovereign debt, step 1 — the data library, the data-quality table and the reference (row 3 of the plan, begun).** The attribution is one division — `exposure ÷ PPP-adjusted GDP (international USD)` (p.144) — and the data is public, which is why this is the cheapest high-coverage class to add to a Sri Lankan book. The standard's own worked example is now the acceptance test, taken verbatim from **Table 10.3-2 (p.202)**: an investor holding $1M of each sovereign's debt attributes **106 tCO₂e** to Singapore (scope 1 excl. LULUCF 61,451,586 tCO₂e ÷ PPP-GDP 579,762 $M) and **91 tCO₂e** to Hong Kong (42,654,105 ÷ 469,182) — the debt-denominator approach gives 196 and 268,264, the 1,369× distortion PPP-GDP exists to remove. `data/pcaf-parta/dq-sovereign.json` holds **Table 5.9-6 (p.147)**: 1a→1 (verified UNFCCC), 1b→2 (unverified), 2→3 (physical energy activity), 3a→4 (sectoral revenue), 3b→5 (proxy country) — its own shape, no 2b and no 3c, held apart from every other class's table. `data/pcaf-parta/sovereign/dataset.json` holds one record per country carrying **scope 1 both including and excluding LULUCF** (a *shall*, p.141; the include/exclude pairing is Table 5.9-2, p.142), PPP-GDP and population, each figure with its source and vintage; Singapore and Hong Kong carry the Table 10.3-2 figures as fixtures (scope 1 excl. LULUCF only, incl. reported absent-with-reason because the standard's example gives only excl.), Sri Lanka is shipped provisional and says so on every row. The set is versioned, dated and checksummed (`data/pcaf-parta/sovereign/MANIFEST.json`, `npm run docs:parta-sovereign-manifest`), the discipline the sector-factor manifest already follows, so a disclosure names the sovereign set it rests on. The public sources are the ones PCAF itself lists (**Table 10.3-4, pp.205–206**): UNFCCC `di.unfccc.int`, Climate Watch, EDGAR (a stated **4-year lag**, GHG 1970–2018), OECD trade-embodied CO₂ for scope 2/3 (CO₂ only), World Bank PPP-GDP. Consumption emissions = production − exported + imported (p.142) is recorded for the engine but not computed here. `GET /v1/pcaf/part-a/reference` now carries the sovereign-debt class, its data-quality table and the held countries. Step 1 of the §5.9 plan | §5.9 pp.140–148; Table 5.9-1 (p.141); Table 5.9-2 (p.142); Table 5.9-6 (p.147); Table 10.3-2 (p.202); Table 10.3-4 (pp.205–206) |
| 2026-09-11 | §2.2, §10 | **Recalculation is a "shall", and the disclosure now prints the entity's own protocol.** Chapter 6 requires a stated base year, a significance threshold and recalculation triggers, and Part A weights and reports them like Part C but on its own settings (the two scopes never merge). A new org-wide `parta_settings` record (migration 0010) holds `baseYear` (null by default — a base year is a claim about history and belongs to the entity, so where none is set the report says so rather than implying the current year), `significanceThresholdPct` (default 5, the GHG Protocol Scope 3 trigger), the recalculation triggers (a domain default phrased for a lending book) and a free-text policy. `GET/PUT /v1/pcaf/part-a/settings` read and set it; the disclosure's Recalculation section prints it from the settings, absent-when-unset. And `recompute()` now reports whether the movement reaches the threshold — a significant movement is a recalculation trigger that, once the lock-and-supersede lifecycle exists, requires a restatement with a recorded reason; today it is reported, not silently absorbed. Step 6 of the §5.2 plan | Chapter 6 (recalculation); GHG Protocol Scope 3 triggers; the Part C recalculation protocol |
| 2026-09-11 | §10 | **The §5.2 engine carries its own conformance matrix, proved by execution.** `src/domains/pcaf-part-a/domain/conformance.js` maps every §5.2 rule — scope, classification, the option-to-score mapping, estimation, the six lines, the third verdict and its checks, the register, the disclosed score and the report — to the code that enforces it and the test that proves it, in the shape Part C and the GCF pipeline already use. The report rules that shipped as a separate `reporting/conformance.js` are folded in, because this is one §5.2 statement, not two. `GET /v1/pcaf/part-a/conformance` serves it; `docs/PCAF-PART-A-CONFORMANCE.md` is generated from it (`npm run docs:parta-conformance`); `tests/pcaf-parta-conformance.test.js` fails on a citation that does not resolve; and `scripts/conformance-evidence.js` runs the §5.2 rules alongside the others under coverage, so a citation that merely resolves is not mistaken for behaviour — 102 of 109 rules across the three matrices proved by execution, one §5.2 scope rule by the absence of a path. Step 5 of the §5.2 plan | the north star's "every module carries its own conformance evidence"; the Part C conformance matrix |
| 2026-09-11 | §4, §10 | **The §5.2 report and the Disclosure Checklist built, and the renderer is now genuinely one.** The generic report machinery — the visual theme, the block vocabulary and the PDF/Word renderers — moved from `src/domains/pcaf-part-c/reporting/` to `src/platform/reporting/report-standard/`, theme injected as a parameter, so Part C and Part A render one document model through one renderer without either domain importing the other. Part C's output is byte-identical (the golden test holds it). Part A's report is built from the register's position: one content model in the DCL's order (Chapter 6, pp.160–174), the Annex 10.2 table, coverage against the whole book, the outstanding-weighted score with scope 3 apart, the factor set and baseline versions named, and the data-checks carried as the limitations section. The twelve Chapter 6 requirements are answered **from the report** (the `partc-checklist` pattern), so an item can fail; the completed checklist is an annex. Step 4 of the §5.2 plan; row 8 of §11 moves to §10 | Chapter 6 (pp.160–174); Annex 10.2 (pp.199–200); the north star's "one renderer" |
| 2026-09-11 | §4 | **A completed Part A checklist can never reach 100%, and that is correct.** The inventory the standard asks a bank to disclose (SLFRS S2 §29(a) gross scope 1/2/3, GRI 305 reduction) is the *entity's own* — this report covers the §5.2 asset class, which is one input to that inventory, not the inventory. So the report says on its face that it is one asset class of a Chapter 6 disclosure, and the checklist items that ask for the whole are answered No with that reason rather than Yes. A checklist that could reach 100% would be claiming to be the disclosure — the same rule the GCF report already follows | Chapter 6 (p.160); GCF reporting's inventory rule |
...
| 2026-09-17 | §1, §3, §11 | **The numerator is the year-end outstanding, never the amount the client asked for; the commitment is recorded, and the undrawn part is reported apart.** Read for the case a relationship manager brings — a client asking for 250 million over five years — the standard is explicit: the position is taken at one fixed point in time, the fiscal year-end (p.33); the outstanding amount is *the value of the debt the borrower owes, disbursed debt minus any repayments*, adjusted annually so the attribution declines to zero when the loan is repaid (§5.2 p.56; §5.3 p.68–69; §5.4 fn 124; §5.6 p.91; §5.7 p.104). So the sanctioned 250 million is the numerator only where it is fully drawn and nothing has been repaid on 31 December; a facility drawn in March and amortising quarterly carries 212.5 million at the first year-end and less at each after it, and each reporting year is a fresh measurement of that year's balance against that year's denominator and that year's borrower emissions — nothing is spread over the tenor. What the standard adds in the Third Edition is §6.2 (pp.169–173): the *undrawn* commitment — total loan commitment minus the drawn amount at the same point in time — may be reported, on the same denominator as the drawn part, as an unweighted figure that **shall** be reported by anyone reporting undrawn and a weighted one (a utilisation factor, method disclosed) that **may** sit beside it, never instead; it is not an asset class but a condition of a loan, it is never aggregated with the drawn figure (p.170), and the products included in the definition shall be disclosed (p.171). An average balance may be used but is reported separately with the method transparent (p.65), which is the footnote 71 check already built. Built from this: a `facility` block on every loan class — sanctioned amount, drawn to date, origination and maturity, the repayment profile — from which the engine schedules the balance at each year-end; the year-end outstanding the form takes from the schedule is recorded as *scheduled* and carries a material finding until the ledger's balance replaces it; the §6.2 undrawn line is computed apart and summed apart; and the life-of-loan attribution is a **projection**, hatched wherever it is drawn, which never enters the position. Table 5.2-1 is untouched: the facility changes no score, because the score is about the emissions data and the numerator is money | PCAF Part A Third Edition p.33, §5.2 p.56 and fn 71, p.65, §5.3 pp.68–69, §5.4 fn 124, §5.6 p.91, §6.2 pp.169–173; CarbonIQ `domain/facility/` |
| 2026-09-17 | §3, §11 | **The path a loan is priced on is asked, not inferred, and the standard's own table answers what would raise the score.** Table 5.2-1 (p.60) prefers Options 1 and 2 over 3 and lists, per option, what each needs; read from the option a loan earned, every row with a lower score is a step the loan could take, and the sentence beside each is what to go back to the borrower for. The Lending Book now asks how the borrower's emissions are known — reported, or not known and estimated from its industry on the held sector factor (3a per unit of revenue at score 4, 3b on the outstanding alone at score 5, footnote 73: no attribution factor) — and shows the engine's answer before Record through a route that writes nothing. The walkthrough carries a second borrower priced this way, because on a Sri Lankan book it is the common case | PCAF Part A Third Edition §5.2, Table 5.2-1 (p.60), footnote 73; CarbonIQ `domain/business-loans/raise.js`, `application/register.js` |
| 2026-09-17 | §11 | **The documents print amounts with their ISO code, separators and scale, and draw the position.** No figure moved: every Part A document now prints tonnes as `12,038.240` and an amount as `LKR 250,000,000 (250 mn)` — the code first and never a symbol, because a symbol is a claim about the currency — and the S2 file draws the position by class, scope 3 apart, the three §29(b)–(d) amounts and the industry table as bars beside the tables that carry the figures. The regenerated goldens differ only in separators, code order, the scale in brackets and the new bar rows | CarbonIQ `src/shared/money.js`, `platform/reporting/report-standard` |
| 2026-09-17 | §11 | **The consolidated file is an SLFRS S2 disclosure in the standard's own order.** The body is entity and basis of preparation, then governance, strategy, risk management, metrics and targets (the inventory with category 15 carried in, the cross-industry metrics, the industry table, the targets), what is outstanding, and a statement of compliance; the PCAF Chapter 6 blocks — coverage, gases, the position by class, methodology, data quality, recalculation, intensity, uncertainty — are the basis of preparation for the category 15 metric and are printed as such in Annexes A and B, unchanged. Reordering moved no figure; the golden's figure and table rows survive as a set. One S2 item had no field — the measurement approach, inputs and assumptions of §29(a)(iii) — and has one now, so the registry covers every paragraph the index names | SLFRS S2 §5–37 (IFRS S2 as adopted); B58–B63; CarbonIQ `reporting/consolidated/` |
