...
const AgentsPage = (() => {
  ...
  async function runAgent() {
    ...
    try {
      ...
      /* A network failure says the backend is unreachable. It used to answer
         with a stored memo instead — a credit recommendation, a carbon
         intensity, a benchmark percentile and a pricing ratchet, all invented,
         behind a `[DEMO MODE]` prefix. That is the failure report-integrity
         exists to prevent, and it survived here because nothing sweeps the
         browser. A reader who copies one figure out of a document loses the
         prefix with it. */
      showOutput('error');
      elErrorMsg.textContent = err instanceof TypeError
        ? 'The API did not respond. No output is shown, because a figure this '
          + 'screen invented is a figure someone could act on.'
