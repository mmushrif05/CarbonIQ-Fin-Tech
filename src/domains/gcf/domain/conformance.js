...
const RULES = [
  ...
  {
    id: 'G-CYCLE-01',
    clause: 'GCF project activity cycle — ten stages from programming to closure',
    rule: 'The record’s stage vocabulary maps onto GCF’s ten stages in GCF’s order, runs past Board approval to closure, and a stage the cycle does not have is refused.',
    implementation: 'src/domains/gcf/domain/cycle.js — CYCLE, STAGE_INFO, stageInfo(), nextStage()',
    test: 'tests/gcf-cycle.test.js › The cycle is GCF’s, in GCF’s order › every record stage maps onto one cycle stage, and the record’s vocabulary is the cycle’s',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-02',
    clause: 'ToR Lot 1, Milestone 4 — systems and procedures to capture data; a pipeline has a time axis',
    rule: 'A stage move is dated into the project’s history with who made it, and the milestone dates that travel with it land on the timeline in the same write; a stage is never overwritten silently.',
    implementation: 'src/domains/gcf/infrastructure/store.js — moveStage()',
    test: 'tests/gcf-cycle.test.js › The cycle over HTTP › a stage move is dated into the history, carries its milestone dates, and answers with the new readiness',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-03',
    clause: 'GCF-2 service standards — six weeks for concept-note feedback, nine months to approval, eleven months to first disbursement',
    rule: 'A date the Fund’s published timing implies is marked projected and names the standard it rests on; a recorded date never is; nothing is projected from nothing.',
    implementation: 'src/domains/gcf/domain/cycle.js — TIMING, projections(), timeline()',
    test: 'tests/gcf-cycle.test.js › Time on the cycle › a projected date is marked projected and names the standard it rests on; a recorded one is not',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-04',
    clause: 'Concept note template v2.2 and funding proposal template — what each stage must contain; Sri Lanka NDA Operation Manual',
    rule: 'What each stage needs is answered from the record as held, partial or missing with the clause and the remedy; it judges nothing, and a gap never refuses a project or a move.',
    implementation: 'src/domains/gcf/domain/readiness.js — REQUIREMENTS, assess()',
    test: 'tests/gcf-cycle.test.js › Readiness is answered from the record and judges nothing › a gap never refuses: assess answers for a record with almost nothing on it',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-05',
    clause: 'Board decision B.32/05 — Simplified Approval Process; Project Preparation Facility guidelines',
    rule: 'SAP eligibility is bounded by USD 25 million of GCF funding and category C or I-3, and PPF support by a tenth of the ask and USD 1.5 million; each answer names the condition that failed.',
    implementation: 'src/domains/gcf/domain/readiness.js — sapEligibility(), ppfEligibility()',
    test: 'tests/gcf-cycle.test.js › Readiness is answered from the record and judges nothing › SAP is bounded by the ask and the safeguards category, and says which condition failed',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-06',
    clause: 'GCF investment framework — six investment criteria and their sub-criteria',
    rule: 'The six criteria are answered as evidence held on the record — evidenced, partial or absent — never as a score, and the three the engine does not score are named unscored.',
    implementation: 'src/domains/gcf/domain/criteria.js — CRITERIA, assess()',
    test: 'tests/gcf-cycle.test.js › The six investment criteria are evidence held, never a score › nothing in the answer is a number out of anything',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-07',
    clause: 'ToR Lot 2 — the pipeline as a whole, read by the bank and the Fund',
    rule: 'The portfolio view composes the emissions model, the gate, the readiness checklist and the criteria and computes nothing of its own: the three carbon boundaries stay on separate keys, direct and indirect beneficiaries are never summed, and the accreditation ceiling applies per project.',
    implementation: 'src/domains/gcf/domain/portfolio.js — portfolio()',
    test: 'tests/gcf-cycle.test.js › The portfolio computes nothing and merges nothing › the results are the emissions model’s own figures, on their separate boundaries',
    status: 'implemented',
  },
  {
    id: 'G-CYCLE-08',
    clause: 'ToR Lot 1, Milestone 4 — illustrative data replaced by the entity’s own, never mixed with it',
    rule: 'The shipped sample is read-only: a change or a move on it is refused with the remedy to adopt the pipeline first, because one edited copy would replace the whole illustrative set with a single record.',
    implementation: 'src/domains/gcf/infrastructure/store.js — recordedOrRefuse(), patch()',
    test: 'tests/gcf-cycle.test.js › The cycle over HTTP › the shipped sample cannot be edited or moved — the refusal says to adopt it first',
    status: 'implemented',
  },
  {
    id: 'G-ACCR-05',
    clause: 'Board decision B.36/10 — DFCC’s accreditation envelope; the entity’s own where recorded',
    rule: 'Every gate reads the entity’s own accreditation once recorded, and the shipped one only until then; the answer says which.',
    implementation: 'src/domains/gcf/infrastructure/store.js — accreditation()',
    test: 'tests/gcf-cycle.test.js › The cycle over HTTP › the entity’s own accreditation, once recorded, is what every gate reads',
    status: 'implemented',
  },
