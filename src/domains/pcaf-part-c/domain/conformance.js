...
const RULES = [
  // ---- Scope ------------------------------------------------------------
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/rollup.js — construction = A4 + A5, reported as the PCAF figure',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/rollup.js — useStage is a distinct traced value; no code path sums it with construction',
  ...
  },
  {
    ...
    implementation: 'Not computed. src/domains/lending/application/pcaf.js handles A1-A3 for lending and is a separate service with no import path into the Part C engine.',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/policy-gate.js — useStageYears() returns 0 for CAR/EAR and the cover period for IDI/Property; an unrecognised type fails closed to construction-only',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/{b1-refrigerant,b4-replacement,b7-water}.js — each returns an explicitly traced zero carrying a GATED assumption naming the policy type',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/beyond-pcaf.js is not imported by rollup.js — the constraint is enforced by the module graph, not by convention. Results surface only under result.beyondPcafAnnex.',
  ...
  // ---- Attribution ------------------------------------------------------
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/attribution.js — one basis per policy type, each recording its equation and both operands',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/attribution.js — net-premium mode deducts ceded premium and records an ATTR_NET_PREMIUM assumption naming the amount',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/index.js — runPartC() computes a single policy against a single project; no cross-project pooling exists in the engine',
  ...
  // ---- Method -----------------------------------------------------------
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/a4-transport.js — a4Material() and a4Total()',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/a5-construction.js — the three sub-modules, summed by a5Total()',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/b1-refrigerant.js; factors in data/factors/refrigerant-{leak,gwp}.json. GWP basis is AR5 100-year and is stated in the factor table.',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/b1-refrigerant.js — three-step priority; each fallback records its basis, and the per-m2 screen is marked a literature assumption rather than a standard',
  ...
  },
  {
    ...
    implementation: "src/domains/pcaf-part-c/domain/b4-replacement.js — replacementCount() = max(ceil(years / life) − 1, 0)",
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/b7-water.js — supply and wastewater factors held separately and summed, so either may be localised independently; the B6 exclusion is stated in the module output',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/b1-refrigerant.js — deMinimisCheck() reports the ratio and always returns excluded: false',
  ...
  // ---- Data quality and disclosure --------------------------------------
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/data-quality.js — option-to-score map; the MVP reports Option 2b (physical activity data with secondary factors) = score 3',
  ...
  },
  {
    ...
    implementation: 'data/factors/*.json — every row carries tier and reference; src/domains/pcaf-part-c/domain/factors.js refuses to return an anonymous value, and GET /v1/pcaf/part-c/factors publishes the whole store',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-registers.js — the Assumptions and Limitations Register is derived from the traced-value tree, so it cannot disagree with the arithmetic. Reported as Annex A in both PDF and Word.',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/data-quality.js — containsForbiddenLanguage(); src/domains/pcaf-part-c/reporting/partc-reports.js refuses to build a report containing endorsement language',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/provenance.js — every engine function returns a traced value; the audit trail in Annex C is generated by walking that tree rather than narrated after the fact',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/data-quality.js maps the six options to their scores and infers the option from the data the run actually consumed, with an explicit override honoured; src/domains/pcaf-part-c/domain/dq-scoring.js reports that score and computes no average of its own.',
    test: 'tests/pcaf-partc-dq-scoring.test.js \u203a The Fisheries run \u203a is Option 2b with score 3 \u2014 a whole number, not an average',
    status: 'implemented'
  },
  {
    id: 'C-DQ-08',
    ...
    implementation: 'src/domains/pcaf-part-c/domain/dq-scoring.js useStageBasis() returns a reason and a set of qualitative statements and no number at all. Nothing downstream \u2014 API, roll-up, report or UI \u2014 reconstructs one.',
    test: 'tests/pcaf-partc-dq-scoring.test.js \u203a The use stage is never scored \u203a no numeric use-stage score exists anywhere in the scoring output',
    status: 'implemented'
  },
  {
    id: 'C-DQ-09',
    ...
    implementation: 'src/domains/pcaf-part-c/domain/dq-scoring.js disclosureStatement() — every clause is read from the result, and limitations are named from the inputs that scored 4 or worse, so a supplied actual removes its own limitation',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/ is pure and deterministic: no network, no clock in any calculation, no LLM in any arithmetic path. Claude classifies, extracts and writes; the engine computes.',
  ...
  // ---- Reporting requirements (Part C ch.6) ------------------------------
  {
    ...
    implementation: 'src/domains/pcaf-part-c/domain/ghg-scopes.js maps each lifecycle stage to a GHG scope once; the emissions split and the data-quality split both read that map, so the two cuts cannot disagree. The report renders both in section 4.',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-portfolio.js _premiumWeighted() produces the disclosed score from each policy\u2019s own option score. No emission-weighted score exists to be quoted by mistake. A policy with no score is excluded from the weighting rather than counted as zero.',
    test: 'tests/partc-portfolio.test.js \u203a Portfolio \u2014 the disclosed data-quality score \u203a is premium-weighted, to two decimals, and says which scale it is on',
    status: 'implemented'
  },
  {
    id: 'C-RPT-03',
    ...
    implementation: 'src/domains/pcaf-part-c/interface/schemas/partc-registry.js holds baseYear, significanceThresholdPct and recalculationTriggers on the entity\u2019s settings, defaulted to the GHG Protocol Scope 3 triggers; section 7 of every report prints them, and says plainly when no base year has been set rather than implying one.',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-checklist.js completes the checklist from the same facts the sections render, so an item cannot answer Yes to something the document does not contain; anything but Yes carries its reason, and the completed checklist is the final annex of both documents.',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/reporting/partc-report-standard.js KYOTO_GASES names all seven with where each arises in a construction value chain; section 3 states the 100-year horizon and IPCC AR5, and discloses the AR4/AR5 difference for R-410A rather than reconciling it silently.',
  ...
  // ---- Annual disclosure -------------------------------------------------
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-disclosure.js — coverage.statement names assessed and in-force policy counts in section 3, and every unassessed policy is listed by name',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-disclosure.js — buildAnnualDisclosure throws NOTHING_TO_DISCLOSE (409) when the year holds no locked assessment',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-disclosure.js — Annex C records assessment id, version, BOQ revision, lock time and locking organisation for every row in section 5',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-comparatives.js — restatementsFor() reports asPreviouslyReported/asRestated per policy with the reason recorded at lock time; compare() carries both bases into the following year',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-comparatives.js — the register reads locked assessments only, so a draft that would move the figure has not moved it',
  ...
  },
  {
    ...
    implementation: 'src/domains/pcaf-part-c/application/partc-comparatives.js — a policy is reported in its inception year, so each year covers different policies; compare() states this with the movement and reports intensity (kgCO2e/m² insured) and emissions-weighted data quality as the comparable measures',
