/**
 * Which data-quality option each scope has earned — the machinery §5.1 and
 * §5.2 share.
 *
 * PCAF scores by the option used to estimate the emissions, and both chapters
 * are specific about the same trap (p.47, p.61): a figure from a data provider
 * is Option 1 only if the provider is relaying what the company reported.
 * Where the provider estimated it, the calculation "would be in line with
 * Options 2 or 3". So the option is read from the BASIS of the emissions
 * figure — what was done to get the number — and never chosen from a list. A
 * caller may claim a different option only with a justification, which is
 * recorded beside the score.
 *
 * Two options per exposure, not one. Option 2a covers scope 1 and 2 only
 * (§5.1 fn 53, §5.2 fn 87); scope 3 must come from another option, and
 * Chapter 6 reports the scope 3 score separately in any case (p.167). Scope 1
 * and scope 2 may themselves rest on different bases — reported scope 1,
 * estimated scope 2 — and the pair is scored on the weaker of the two, with
 * both recorded.
 *
 * This module is built per table rather than bound to one. §5.1 and §5.2 carry
 * the same seven options at the same seven scores, which is a fact about those
 * two chapters and not about the standard: the real-estate tables put the same
 * labels at different scores. So the logic is shared and the TABLE is not —
 * each class passes its own, and asking the wrong one is impossible rather
...
const ESTIMATED_BASES = new Set([
  'energy-activity', 'production-activity', 'revenue-sector', 'assets-sector', 'turnover-sector',
  'fi-sectoral-distribution', 'fi-economy-wide',
]);

/**
 * Bind the machinery to one asset class's table.
 *
 * @param {any} table   the checked Table 5.x-1/2 for this class
 * @param {Object} refs
 * @param {string} refs.reference        cited on every option this class resolves
 * @param {string} refs.scope2aFootnote  the footnote number restricting 2a to scope 1 and 2
 * @param {string} refs.alternativePage  the page permitting an alternative option
 * @param {string} [refs.verifiedFootnote]
 */
function build(table, refs) {
  const BY_BASIS = {};
  ...
  function optionForScope(s, scope) {
    ...
    /* A provider figure must say what the provider did. */
    if (basis === 'provider') {
      ...
      if (m === 'relayed' || m === 'reported') basis = 'provider-relayed';
        ...
        throw refuse('PROVIDER_METHOD_REQUIRED',
          ...
          + 'method is not stated. PCAF: a provider figure is Option 1 only where it relays what the '
    ...
    if (!row) {
      throw refuse('EMISSIONS_BASIS_UNKNOWN',
        `Scope ${scope} basis "${s.basis}" is not one ${table.table} scores. Use one of: `
        + `${Object.keys(BY_BASIS).join(', ')}.`);
    }

    /* Energy-based estimation cannot reach scope 3. */
    if (row.scopes && !row.scopes.includes(scope)) {
      throw refuse('OPTION_NOT_APPLICABLE_TO_SCOPE',
        `Option ${row.option} cannot estimate scope ${scope} (footnote ${refs.scope2aFootnote}: quality scoring `
        + `for Option ${row.option} is applicable to scope 1 and 2 only). Supply scope 3 on another basis, or `
        + 'leave it absent with the reason, which the report will carry.');
    }
    ...
    if (row.option === '1a' && !s.verifier) {
      throw refuse('VERIFIER_REQUIRED',
        ...
        + `in line with the GHG Protocol and verified by a third-party auditor${refs.verifiedFootnote ? ` (footnote ${refs.verifiedFootnote})` : ''}. `
        + 'Name the verifier, or enter the figure as reported-unverified (Option 1b).');
    }

    /* An alternative method is allowed only with an explanation. */
    if (row.option === 'alt' && !s.justification) {
      throw refuse('ALTERNATIVE_JUSTIFICATION_REQUIRED',
        ...
        + `deviation shall be accompanied by an explanation (${refs.alternativePage}). Supply a justification.`);
    }
  ...
  function deriveOptions(em = {}) {
    ...
    if (!o1 || !o2) {
      throw refuse('SCOPE_1_2_REQUIRED',
        'Scope 1 and scope 2 of the borrower or investee are required: a financial institution shall report '
        + `the absolute scope 1, 2 and 3 emissions of borrowers and investees (${REF}). Each needs a figure `
        + 'and the basis it rests on.');
    }
    ...
    /* No factor for 3b/3c. If any scope needs a factor and another does not,
       both paths run; the roll-up reads each line's own flag. */
  ...
  return { optionForScope, deriveOptions, reconcileClaim, TABLE: table };
}
