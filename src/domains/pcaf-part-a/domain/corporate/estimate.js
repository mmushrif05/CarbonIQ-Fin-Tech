/**
 * The borrower's or investee's emissions, where they have to be estimated —
 * the Annex 10.1 equations §5.1 and §5.2 share.
 *
 * Annex 10.1 gives one equation per option, and this module runs exactly
...
/**
 * Bind the equations to one chapter's annex table and footnote numbers.
 *
 * @param {Object} refs
 * @param {string} refs.reference          the Annex 10.1 table for this class
 * @param {string} refs.tableName          the class's own data-quality table
 * @param {string} refs.noFactorFootnote   the footnote saying 3b/3c yield no attribution factor
 * @param {string} refs.indicatorFootnote  the footnote permitting an indicator other than revenue
 */
function build(refs) {
  ...
  switch (option) {
    case '2a': {
      ...
      if (activity.processEmissions_tCO2e === undefined) {
        assumptions.push('No process emissions declared; taken as 0. Option 2a requires relevant '
          + `process emissions to be added (${refs.tableName}).`);
      }
      return traced({
      ...
      if (indicator !== 'revenue') {
        if (!activity.indicatorReason) {
          throw refuse('INDICATOR_REASON_REQUIRED',
            `Option 3a uses ${indicator} in place of revenue. Footnote ${refs.indicatorFootnote} permits another indicator where revenue is `
            + 'unsuitable, provided the reasoning is made transparent. State indicatorReason.');
        }
        assumptions.push(`${indicator} used in place of revenue as the financial indicator: ${activity.indicatorReason} (footnote ${refs.indicatorFootnote}). The data quality score is unaffected.`);
      }
      return traced({
    ...
    case '3b': {
      ...
      return Object.assign(traced({
        ...
        reference: `${REF}; footnote ${refs.noFactorFootnote}`,
        assumptions: [...inf.assumptions,
          ...
          + `multiplied directly by a sector-average intensity (footnote ${refs.noFactorFootnote}).`],
    ...
    case '3c': {
      ...
      return Object.assign(traced({
        ...
        reference: `${REF}; footnote ${refs.noFactorFootnote}`,
        assumptions: [...inf.assumptions,
          `No attribution factor: EVIC or total equity plus debt is not known (footnote ${refs.noFactorFootnote}).`],
      }), { alreadyAttributed: true });
    }
