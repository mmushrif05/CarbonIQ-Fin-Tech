 ...
 *
 * It presents the same five operations `services/partc-store.js` has always
...
function projection(collection, fields) {
  ...
  for (const f of fields) {
    if (f.includes('[]')) {
      throw Object.assign(new Error(`"${f}" picks inside an array, which only a stored projection can do; declare one in platform/database/collections.js.`), { statusCode: 400, code: 'PROJECTION_NOT_STORED' });
    }
