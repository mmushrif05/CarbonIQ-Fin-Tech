...
At the time of writing: 138 suites, 2,763 tests on PostgreSQL.
...
_198 operations across 163 paths. Generated from `docs/openapi.json`, which is itself generated from the router — so a route that is not here does not exist. `docs/API-SCOPES.md` has the scope each one requires._
...
| `GET` | `/v1/pcaf/part-a/conformance` | PCAF Part A §5.2 conformance matrix — clause, implementation and proving test per rule |
