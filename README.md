...
At the time of writing: 179 suites, 3,333 tests on the in-memory store.
...
_233 operations across 195 paths. Generated from `docs/openapi.json`, which is itself generated from the router — so a route that is not here does not exist. `docs/API-SCOPES.md` has the scope each one requires._
...
| `GET` | `/v1/auth/preview` | Whether the sample book can be opened here |
| `POST` | `/v1/auth/preview` | Open the sample book |
...
| `PUT` | `/v1/pcaf/part-a/book` | State the total loans and investments for a reporting year |
| `GET` | `/v1/pcaf/part-a/book/{year}` | The reporting entity's stated total loans and investments for a year |
| `POST` | `/v1/pcaf/part-a/business-loans/assess` | PCAF Part A §5.2 financed emissions for one business loan or unlisted equity holding |
| `POST` | `/v1/pcaf/part-a/business-loans/portfolio` | Roll up a book of §5.2 exposures and rank what to fix first |
| `GET` | `/v1/pcaf/part-a/classes` | The Part A asset classes this one exposure register holds |
| `GET` | `/v1/pcaf/part-a/climate/reference` | The vocabularies and the SLFRS S2 items the entity answers |
| `GET` | `/v1/pcaf/part-a/conformance` | PCAF Part A conformance matrix (§5.2 and §5.9) — clause, implementation and proving test per rule |
| `GET` | `/v1/pcaf/part-a/disclosure/{year}` | The annual PCAF Part A §5.2 disclosure — JSON, PDF or Word |
| `GET` | `/v1/pcaf/part-a/exposures` | A reporting year's exposures, a page at a time |
| `POST` | `/v1/pcaf/part-a/exposures` | Record one exposure in the register |
| `POST` | `/v1/pcaf/part-a/exposures/preview` | What the engine makes of an exposure before it is recorded — a read, stores nothing |
| `DELETE` | `/v1/pcaf/part-a/exposures/{exposureId}` | Remove an exposure from the register |
| `GET` | `/v1/pcaf/part-a/exposures/{exposureId}` | One exposure, with the input it was computed from and its whole trace |
| `PUT` | `/v1/pcaf/part-a/exposures/{exposureId}` | Change a recorded exposure; the engine reruns over the new input |
| `POST` | `/v1/pcaf/part-a/exposures/{exposureId}/recompute` | Rerun the engine over the input already held, and say what moved |
| `POST` | `/v1/pcaf/part-a/exposures/{exposureId}/report` | The per-exposure §5.2 report for one recorded exposure — JSON, PDF or Word |
...
| `POST` | `/v1/pcaf/part-a/facility/schedule` | The balance a facility’s repayment schedule expects at a date — a read, stores nothing |
...
| `POST` | `/v1/pcaf/part-a/real-estate/assess` | PCAF Part A §5.4 / §5.5 financed emissions for one property (CRE or mortgage) |
| `GET` | `/v1/pcaf/part-a/reference` | Part A asset classes, archetypes and the data-quality options for each |
| `GET` | `/v1/pcaf/part-a/settings` | The reporting entity's recalculation protocol — base year, significance threshold and triggers |
| `PUT` | `/v1/pcaf/part-a/settings` | Set the reporting entity's recalculation protocol |
| `POST` | `/v1/pcaf/part-a/settings/climate/illustrative` | Load the illustrative SLFRS S2 climate facts for a trial |
| `POST` | `/v1/pcaf/part-a/sovereign/assess` | PCAF Part A §5.9 financed emissions for one sovereign bond or loan |
| `GET` | `/v1/pcaf/part-a/sovereign/disclosure/{year}` | The annual PCAF Part A §5.9 sovereign-debt disclosure — JSON, PDF or Word |
| `GET` | `/v1/pcaf/part-a/sovereign/exposures` | A reporting year's sovereign exposures, a page at a time |
| `POST` | `/v1/pcaf/part-a/sovereign/exposures` | Record one sovereign exposure in the register |
| `DELETE` | `/v1/pcaf/part-a/sovereign/exposures/{exposureId}` | Remove a sovereign exposure from the register |
| `GET` | `/v1/pcaf/part-a/sovereign/exposures/{exposureId}` | One sovereign exposure, with the input it was computed from and its whole trace |
| `PUT` | `/v1/pcaf/part-a/sovereign/exposures/{exposureId}` | Change a recorded sovereign exposure; the engine reruns over the new input |
| `POST` | `/v1/pcaf/part-a/sovereign/exposures/{exposureId}/recompute` | Rerun the sovereign engine over the input already held, and say what moved |
| `POST` | `/v1/pcaf/part-a/sovereign/exposures/{exposureId}/report` | The per-holding §5.9 report for one recorded sovereign exposure — JSON, PDF or Word |
| `GET` | `/v1/pcaf/part-a/sovereign/position/{year}` | The sovereign reporting-year position, rolled up from the recorded exposures |
| `GET` | `/v1/pcaf/part-a/sovereign/years` | The reporting years the sovereign book holds exposures for |
| `POST` | `/v1/pcaf/part-a/starter` | Load the illustrative starter book into this organisation, across every built Part A class |
| `GET` | `/v1/pcaf/part-a/starter/example` | One illustrative business loan, every field filled, for the walkthrough to record live |
| `GET` | `/v1/pcaf/part-a/storage` | What this deployment can actually persist for Part A |
| `GET` | `/v1/pcaf/part-a/years` | The reporting years this book holds exposures for |
...
| `GET` | `/v1/gcf/gaps` | The gap register — what is blocking each project, and who holds the key |
...
| `GET` | `/v1/gcf/pipeline/example` | An example candidate, served to pre-fill the intake form — never recorded by this route |
| `POST` | `/v1/gcf/pipeline/install-starter` | Load the DFCC starter projects (real, editable) into this organisation |
...
| `PATCH` | `/v1/gcf/pipeline/{id}` | Change part of a recorded candidate; the merged record is held to the whole schema |
| `GET` | `/v1/gcf/pipeline/{id}/assessment-report` | The GCF assessment report for one candidate — JSON, PDF or Word |
| `GET` | `/v1/gcf/pipeline/{id}/readiness` | One candidate against the GCF project cycle — what it holds, what is missing, what is next |
| `GET` | `/v1/gcf/pipeline/{id}/return` | The return-to-sponsor state — the gap list, the resubmission comparison and the returns history |
| `POST` | `/v1/gcf/pipeline/{id}/return` | Return a validated assessment to the sponsor, snapshotting the gap list |
| `GET` | `/v1/gcf/pipeline/{id}/return-letter` | The return-to-sponsor letter for one candidate — JSON, PDF or Word |
| `POST` | `/v1/gcf/pipeline/{id}/stage` | Move a recorded candidate to another stage of the GCF project cycle, dated, into its history |
| `GET` | `/v1/gcf/pipeline/{id}/validation` | The assessor validation of a candidate — state, per-criterion ratings, recommendation and history |
| `POST` | `/v1/gcf/pipeline/{id}/validation` | Record an assessor validation change — start review, rate, recommend, validate or reopen |
| `GET` | `/v1/gcf/portfolio` | The pipeline as a portfolio — money, envelope, gate, results, stages, readiness and what is next |
| `POST` | `/v1/gcf/precheck` | Read the sponsor pre-check answers into an advisory (stores nothing) |
...
| `GET` | `/v1/gcf/report` | SLFRS S1/S2 and GRI lines, with what it could not state — JSON, PDF or Word |
...
| `GET` | `/v1/baselines/for/{assetClass}` | The baselines one asset class reads, each with the figure in force and where it came from |
...
| `GET` | `/v1/projects/{projectId}/monitoring` | Every monitoring entry recorded against a project, priced, with the year-on-year comparison |
