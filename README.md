...
At the time of writing: 155 suites, 2,995 tests on PostgreSQL.
...
_217 operations across 179 paths. Generated from `docs/openapi.json`, which is itself generated from the router — so a route that is not here does not exist. `docs/API-SCOPES.md` has the scope each one requires._
...
| `GET` | `/v1/pcaf/part-a/position/{year}` | The reporting-year position, rolled up from the recorded exposures |
