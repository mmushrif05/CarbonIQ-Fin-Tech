...
Both routes are `read`-scoped and store nothing: the same request twice returns the same answer and issues no id. The persisted exposure register is deliberately not built yet, and `docs/PCAF-PART-A-BUSINESS-LOANS.md` §7 says so rather than leaving it to be found.
