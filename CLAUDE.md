...
npm run docs:parta-manifest        # The same for data/pcaf-parta/MANIFEST.json — Part A's sector factor library
...
npm run test:e2e         # Build, then drive the built dashboard in Chromium (PW_CHROMIUM names a local binary)
...
src/
  ...
  platform/                 never imports a domain
    ...
    database/               the storage seam (store.js) + one adapter per store (adapters/), PostgreSQL
                            client/document store/migrator/audit chain,
...
**This table is a reader's selection, not the surface.** The surface is 184
operations across 153 paths, generated from the router into `docs/openapi.json`
...
| `GET/POST` | `/v1/auth/preview` | Whether the sample book can be opened here · open it with an address alone. Read-only, and the address goes on the register |
| `GET` | `/v1/auth/preview/signups` | Who has asked to see the product — requires the `admin` scope |
...
| `GET` | `/v1/pcaf/part-a/disclosure/:year` | The annual §5.2 disclosure — JSON, PDF or Word; Chapter 6 order; a year with no exposures is a 409 |
| `POST` | `/v1/pcaf/part-a/exposures/:id/report` | The per-exposure §5.2 report — JSON, PDF or Word; a read, stores nothing |
| `GET` | `/v1/pcaf/part-a/conformance` | The §5.2 conformance matrix — rule → implementation → proving test |
| `GET/PUT` | `/v1/pcaf/part-a/settings` | The reporting entity's recalculation protocol — base year, significance threshold, triggers |
...
**Preview access — the sample book, opened by an address (`src/platform/auth/preview.js`, `docs/AUTHENTICATION.md`).** `POST /v1/auth/preview` takes an email address and nothing else and answers with a session. It is the third and last route on the surface carrying no credential, and it carries none for a reason arrived at from the opposite direction to sign-in's: **there is nothing to authenticate.** The address is recorded, not believed. What comes back is bounded rather than trusted — a session holding `read`, in an organisation whose only records are a sample book — so it grants strictly less than a sign-in grants somebody who already has an account.

**One shared account, one register row per address.** An account per visitor is wrong twice: a public form writing into `users` is an unbounded write on the table holding every real person, and `users` keys on the address, so the first time somebody at a bank that already has an account typed their own address in, the route would have to either refuse — announcing to an anonymous caller that the address is registered, which is precisely the oracle `POST /v1/auth/login` goes to lengths to avoid — or touch their real account. So every visitor is admitted on one shared account (`preview@carboniq.invalid`, role `viewer`, organisation `preview`) and each holds their own session row, which is what expires and can be revoked; who asked lives in `preview_signups` (migration `0007`). The two facts are kept apart deliberately: removing an account must not erase the fact that the question was asked. The cost is stated rather than hidden — an audit line for a preview read names the preview account, not the visitor, which is accurate rather than lossy and cannot mislead anyone about a book, because the only book a preview session can reach is the sample one.

**The isolation is a partition that is empty, not a check that refuses.** Every read at the seam takes the organisation as its second argument and is partitioned on it, so an organisation whose only records are the sample book can only ever return the sample book; `tests/preview-access.test.js` proves it by writing a record into another organisation and showing it is invisible with no rule firing. `scopesForRoleLevel(20)` resolves to `['read']`, so a preview session cannot write, cannot lock and cannot run an agent — the last of which also means a public form cannot spend the deployment's AI budget.

**`viewer` is a role of its own rather than a reuse of `auditor`**, because the two make different claims: an auditor is a named person a bank appointed to read its real book; a preview visitor typed an address into a form. Labelling the second as the first would put *Auditor* beside a marketing address on the Accounts screen — the same class of quiet mislabelling as an audit chain that was cryptographically sound and attested to a self-declared identity.

**The register (`GET /v1/auth/preview/signups`) requires `admin`** — one bar above the `read` every other list needs, because it is the most personal thing this deployment holds. A returning address is **one row with a count**, not four rows: "asked once in March" and "has come back four times this week" are different facts about one address and only the second is worth acting on. No IP is kept; the register holds what a visitor chose to give.

**The sample book is the baselines that already exist, plus one install.** Capital and GCF fall back to `data/capital/book.json` and `data/gcf/pipeline.seed.json` for an organisation that has recorded nothing, so they need no seeding; Part C has no such baseline, so its demo book is installed into the preview organisation on first admission. The platform never imports a domain, so `src/server.js` — a composition root — hands the installer in, the same shape as `require('./jobs')`. The install is sequentially idempotent and the one gap is written down rather than left to be found: two visitors pressing the button in the same instant on a never-previewed deployment could both seed, duplicating the sample book. `PREVIEW_ACCESS=off` closes the door and `GET /v1/auth/preview` then says so with the remedy; on is the default, deliberately, because what the route can do is bounded by construction.

**The screen says which book it is, on every page.** A preview session is recognised from the role the server put on it rather than a flag the browser was handed — so it survives a reload and cannot drift from what the server will allow — and `ui/js/auth.js` renders a neutral strip reading *"Sample book — illustrative records, read-only."* It is a provenance label and not a warning, because amber in this product means something the reader has to act on. The sidebar answers a preview session from a named page list rather than a level: the matrix asks "is this person senior enough to do the work on this screen", which for a read-only visitor is the wrong question — the PCAF screens sit at 60 because an analyst runs assessments on them, not because reading one is privileged, and bending the visitor's level up to clear that bar would clear every other bar at or below it.

**The offer is a card of its own, and it shipped as a panel inside the sign-in card.** `_showPane()` hides that whole card to show another, so on a deployment holding **no accounts with a bootstrap token set** — which is every deployment on its first day — the sign-in screen's two independent checks fought each other: `_checkPreview()` revealed the offer, `_checkFirstRun()` replaced the card it was nested in, and a visitor watched the sample book appear and go. Which of the two a visitor saw depended only on which answered first. `_applyPreview()` now decides it from both facts at once so neither answer can overwrite the other, and the offer is withheld only on the change-password pane, where opening a sample book would replace the session the holder is part-way through taking ownership of. Every browser journey was green while the live site did this, because the Playwright server sets no `ADMIN_BOOTSTRAP_TOKEN` and so never draws the pane that caused it; `e2e/preview-access.spec.js` stubs that one answer, in both orders, and nothing else.
...
**Structure (`tests/structure.test.js`, `docs/FRONTEND.md`, `docs/ENVIRONMENTS.md`).** No source file is over 500 lines: the nine that were are split along their seams into sibling modules behind a barrel with the same path and exports (`partc-report-standard.js` → `report-standard/`, `partc-theme.js` → `theme/`, `reports.js` → `reports/`, the agent routes and schemas → `agent/`, `pcaf-partc.js` → `pcaf-partc/`, `partc-methodology.js` → `methodology/`, `capital-metrics.js` + `capital-pipeline.js`, `decision-engine.js` + `decision-tiers.js` + `decision-constants.js`), and the test refuses a new one. **Type checking is on by pragma** (`npm run typecheck`, `jsconfig.json`): `// @ts-check` on every file under `src/platform`, `src/shared`, the roots and the Netlify functions — required by the test — and on every clean file elsewhere; the files still to join are `docs/TYPECHECK-WORKLIST.md`, held to the tree, and a file joins when its errors are fixed, never by pragma. A thrown error that carries a status, code or remedy is an `AppError` (`src/shared/types.js`). **The frontend has a build**: `npm run build:ui` minifies `ui/` into `dist/ui`, which is what Netlify publishes, paths preserved, a manifest beside it; `npm run test:e2e` drives the built output in Chromium through five journeys, in CI. **One Content Security Policy** (`src/platform/http/csp.js`) is sent by helmet and by `netlify.toml`, held to each other by a test; it closes foreign scripts, framing, plugins, base and form targets, and says in its own source that inline scripts stay open until the forty-seven inline handlers are listeners. The markdown library is vendored, not fetched from a CDN. **Environments**: production on `main`, a production-shaped `staging` branch context with its own secrets, deploy previews on pull requests; the CI `gate` job over every check — two Node versions, PostgreSQL, lint, type check, audit at the high level, browser tests — is the status to require on `main`. **The AI layer never reads a request**: `platform/http/deadline.js` reads the invocation's remaining time and hands the figure to `platform/ai/deadline.js`. The phase's exit criterion is a test that writes a cross-domain import, shows the architecture check failing on it, and removes it.
...
**Client factor overrides are an argument, not a global.** `runPartC(input, { overrides })` holds them in an `AsyncLocalStorage` scope for the duration of that one call. They used to be a module-level `let` in `domain/factors.js`, set by the HTTP handler before the calculation and cleared in a `finally` — so a throw left them standing, and two resumes in one container shared one global, which is one insured party's quarry certificate pricing another party's concrete.
...
**PCAF Part A (financed emissions) — built, and the surface is live.** This paragraph read *"planned, not built"* over 3,271 routed lines: `src/domains/pcaf-part-a/domain/` holds attribution, denominators, options, estimation, generation, impact, provenance and the listed-equity denominators, and `GET /v1/pcaf/part-a/reference` and `POST /v1/pcaf/part-a/assess` have been serving them. What follows is the reference material it was built from, and it stands. `docs/PCAF-PART-A-SOURCES.md` holds the working reference: the Chapter 6 reporting requirements extracted from PCAF's **Disclosure Checklist (May 2025)** with page cites, the avoided-emissions and forward-looking-metrics supplement (Dec 2025), and a list of what is still missing. The **Third Edition (2 Dec 2025)** expands Part A from seven to ten asset classes and adds Use of Proceeds, Securitizations, Sub-Sovereign Debt and IFRS-aligned undrawn commitments; the DCL we hold predates it and still says seven. The **Third Edition is now in the repository** (`PCAF-PartA-2025-V3-15012026.pdf`) and `docs/PCAF-PART-A-BUILD-SPEC.md` holds the Chapter 5 study and the module structure. **The finding that matters most: Part A's option-to-score mapping is not uniform across asset classes** — read against all eight tables (`docs/PCAF-PART-A-RESEARCH.md` §3) Option 2a is score 2 in the corporate classes and 3 in real estate, 2b is 3 and 4, Option 3 is 5 in real estate and 4 for sub-sovereigns, and 1b is score 1 for motor vehicles alone: five distinct shapes across eight tables. There is therefore no global option→score lookup; the score resolves as **(asset class, option) → score** from a table per class. Reusing Part C's `2b = 3` here would be wrong for some classes, silently. Also: the six original asset classes carry the *Built on GHG Protocol* mark, but the second- and third-edition additions **have not been reviewed by the GHG Protocol**, so a conformance statement must not blur the two. Two more things not to get wrong: Part A weights the disclosed data-quality score by **outstanding amount** (p.128) where Part C weights by **premium**, so the two engines must not share a weighting function; and avoided emissions are reported **separately** from the scope 1/2/3 inventory and never netted against it (p.126). Part A lives in `src/domains/pcaf-part-a/domain/`, separate from `src/domains/pcaf-part-c/domain/` — three scopes, never merged, and `tests/architecture.test.js` fails the build on an import between them. The lending module's old claim to be "PCAF v3" is gone (see *The glossary found a fourth 1–5 scale* below). **Three of ten asset classes are built** — listed equity and corporate bonds, project finance, and **business loans and unlisted equity (§5.2)**, the largest class on a Sri Lankan book; `docs/PCAF-PART-A-RESEARCH.md` holds the study of all ten, what each requires, what is not built, and the order to build it in. **It is the reference guide, not a snapshot**: a Part A section is built from it, and a finding — a rule read off the standard, a worked example that reproduces, a correction to something we had written — is recorded there with its page cite *before* the code that acts on it, and logged in its §12. A finding that lives only in a commit message is one the next person re-derives.

**§5.2, and the rule the rest of Part A is built on (`src/domains/pcaf-part-a/domain/business-loans/`, `docs/PCAF-PART-A-BUSINESS-LOANS.md`).** Business loans and unlisted equity is the largest class on a Sri Lankan commercial bank's book and the one where almost no borrower reports its emissions. The engine is `classify` (the Figure 5-1 gate, and which of the two denominators applies) → `numerator` → `denominator` → attribute → option → estimate → six lines; `domain/corporate/` holds the machinery §5.1 and §5.2 genuinely share, **bound per chapter** to its own table and footnote numbers, because the seven options at seven identical scores are a fact about these two chapters and not about the standard — the real-estate tables put the same labels at different scores, and a reviewer checking a business loan must be sent to §5.2's footnotes. The anchor test is the standard's own worked example (Tables 5.2-2/5.2-3, p.63) reproduced at 6,100 / 1,260 / 10,000 / 2,200 / 7,250 / 600 across the six lines.

**The engine never blocks a number; it blocks a claim (`domain/corporate/findings.js`).** An engine that only refuses is a gate, and a gate on five thousand SME loans is a product nobody finishes onboarding; an engine that never refuses will print a score of 2 on evidence worth a 5. So there is a third verdict. A **refusal** is for an input that would make the arithmetic wrong or the figure uncitable — a holding larger than the company, a numerator and denominator on different dates, a factor with no publisher, a score of 1 with no named verifier — and there is always a way forward from one, the standard's own: Option 3b needs nothing but the outstanding amount and a sector factor and yields a figure at score 5. A **finding** is for an input that yields a correct figure at a correctly lower score; it stops nothing, changes no figure, travels into the report, and is **material** where a reader of the figure has to be told or **advisory** where it changes nothing they would act on. A finding without a remedy is refused at construction, because a finding that names no fix is a complaint.

**The checks are what a standard cannot give a bank.** PCAF sets the method and says almost nothing about whether a book's data is telling the truth. Each check follows the rule the GCF engine already uses — where an independent path to the same figure exists, recompute and report the divergence; where none exists, say so with the reason — so `FN71_AVERAGE_NOT_HELD` and `INTENSITY_BAND_NOT_HELD` are findings rather than silence, and a check that passes because it had nothing to check cannot happen. The one worth dwelling on is **footnote 71**: only the year-end balance counts, which is the rule and is applied, but a revolving facility drawn to a quarter of its own annual average on 31 December reports a quarter of what the bank actually financed, and nothing in the standard catches it. Three thresholds are CarbonIQ's rather than PCAF's — the 25% "major" movement, the two-year emissions lag, the three-year factor vintage — and each says so on the finding that uses it and is settable per request.

**The improvement plan is the answer to the question a bank asks the day after its first disclosure.** A score is a measurement, not a task list, and "improve your data quality" is not a plan. `POST /v1/pcaf/part-a/business-loans/portfolio` returns the six lines per group, the disclosed score weighted by outstanding amount with scope 3 apart, financial-sector borrowers rolled up separately (p.56) — and then every finding grouped by the sentence that would clear it, and every option band ordered by **outstanding × score points above the target**, because ordering by count sends a bank to two hundred small borrowers before the one exposure carrying a fifth of the book. The default target is score 2, the borrower's own unverified figure, because score 1 needs third-party verification and that is the borrower's decision rather than the lender's. Every figure in the plan is a **scenario**, run through the same weighting the disclosure uses and never presented as the reported score — the discipline the capital forecast follows with hatching.

**The exposure register (migration `0008`, `src/domains/pcaf-part-a/application/register.js`).** §5.2 shipped as two stateless reads, so a book had to be posted whole on every call — right for a pilot, wrong for five thousand loans, and it made two things impossible rather than inconvenient. Coverage could not be stated, because coverage is assessed outstanding over **the whole book** (DCL p.124) and a posted body is only ever what somebody chose to send. And nothing could be traced, because a disclosed figure has to lead back to an exposure that still exists. `parta_exposures` holds one row per exposure per reporting year; `parta_book` holds one row per year carrying the entity's own total loans and investments, and it is a separate table because it is a different fact with a different owner — folding it into a settings blob would make "we have not stated it" and "it is zero" indistinguishable, and coverage against a zero book is unanswerable rather than 100%.

**Both halves are kept, and nothing recomputes on read.** A row holds the input the bank keyed *and* the result the engine computed, with the instant and the standard edition beside them. Keeping only the input would mean a factor correction silently rewrote a figure somebody had already been shown; keeping only the result would mean nobody could see what produced it. `POST /v1/pcaf/part-a/exposures/:id/recompute` reruns the engine over the input already held and **reports what moved**, so a change in a factor is a decision somebody takes rather than something that happens to them. There is no lock and no supersede: nothing publishes from this register yet and a half-built lifecycle is worse than none, so `lock()` refuses with a 501 naming the step that builds it while the `status` column sits in 0008 so that step needs no migration.

**One loan, once — said in the database (migration `0009`), and a recomputation that compares every line.** Nothing in 0008 stopped one facility being recorded twice in one year by two people or one double press, and the position would have carried both. `account_number` is a generated column over the bank's own facility reference with a **partial** unique index on `(org_id, reporting_year, account_number)` — partial because two exposures recorded without a reference are not thereby the same loan. The service refuses first with a `409 DUPLICATE_LOAN` naming the exposure that already holds the reference and both ways forward; the index closes the race the service cannot, exactly as Part C's one-locked-assessment-per-policy-year index does. And `recompute` used to compare the headline alone, so a movement in scope 3, removals or a data-quality score with an unmoved scope 1 and 2 read as *nothing moved*; all seven lines and both scores now travel with before, after and whether each moved, and the note names what did.

**The Lending Book (`ui/pages/parta-register.html`, `ui/js/parta-register.js`).** The §5.2 screen over the register, and it computes nothing — every figure is one the register returned, the rule the Fund Desk established, and `tests/parta-register-ui.test.js` holds it there. It reads in the order a bank reads its own book: the position, with each score a category (1 highest, 5 lowest, never a fraction); the book total, stated from the screen so coverage becomes a percentage without leaving it, beside the groups with financial-sector borrowers apart (p.56); what to fix first, every projected score marked *scenario*; and the exposures, each opening into its lines, its attribution with the equation the engine ran, and its findings inline with what clears them. Recording is a form and no arithmetic — the engine runs before anything is written and a refusal reaches the screen with its clause — and every write control is hidden for a preview session as a courtesy, the server's `read` scope being the control. A preview visitor sees a six-exposure FY2024 sample book installed by the same installer that seeds Part C's, chosen so each shape the checks can take is on screen: the footnote 71 overdraft, the stale sector factor, the EVIC-attributed listed borrower, the development-finance company whose deposits are excluded from its debt, the lagging emissions year. Two of the four mechanical rules were both needed here: `.partc-panel` had to be allowed to shrink (a grid item's `min-width` is `auto`, and the groups table set the panel at 599px inside 430px with its scroll box already in place), and a `max-content` label column needed a floor of zero. `e2e/parta-register.spec.js` seeds a book through the API and drives all of it, then asserts the page never widens.

**The sector factor library, and the bands that are a baseline (`src/domains/pcaf-part-a/domain/sector-factors.js`, `data/pcaf-parta/`, `application/plausibility.js`).** Every Option 3 factor used to arrive on the request, and `INTENSITY_BAND_NOT_HELD` was the usual answer rather than the rare one. The library holds one row per sector of a closed vocabulary — ISIC Rev.4 sections with the divisions a Sri Lankan book actually holds beneath them, because *a paddy-rice factor, not an agriculture factor* is the standard's own example (p.62) — and an Option 3 line with no factor takes the held one for `counterparty.sectorKey`, per unit of revenue for 3a and 3c and per unit of assets for 3b, the second derived from the first and the sector's asset turnover rather than held as a number that could disagree. A free-text sector mapped by name is recorded on the trace, never silently; a sector not held is a refusal naming the vocabulary, never the nearest sector; a factor held in LKR is never applied to an exposure in USD. The result carries `factorRelease` — table, version, rows used, SHA-256 — so a disclosure names the set it rests on; `data/pcaf-parta/MANIFEST.json` is that release committed (`npm run docs:parta-manifest`, held to the table by a test and regenerated in the CI `docs` job), kept apart from Part C's because the two scopes never merge. **Every shipped row is provisional and says so**: order-of-magnitude figures from published global EEIO ranges at an indicative rate, not a licensed extraction and not a Sri Lankan measurement.

The **sector intensity bands** the plausibility check reads are not in the library and not on the request: they are a baseline (`sector_intensity_tCO2e_per_million_revenue`, shape `sector_bands`, `docs/BASELINE-GOVERNANCE.md`), because PCAF sets no such test and a band is therefore regional judgement — scoped global → country → organisation, released by an administrator, superseded only with a recorded reason. `application/plausibility.js` resolves it on the way into the engine and the finding cites the version it was checked against; the shipped LK set says *illustrative dataset* on the finding itself. The band is applied at call time and never written into the stored input, so a recomputation reads the band in force now and the movement reports the findings that changed beside the lines that did not. And `factorVintageYears` had been declared as one of CarbonIQ's three thresholds with nothing checking it; `FACTOR_VINTAGE_STALE` is the check, firing where an economic factor's vintage sits at or beyond the threshold behind the reporting year and no deflator was applied (Box 6.1-5, p.167).

**The §5.2 disclosure, and one renderer for two scopes (`src/domains/pcaf-part-a/reporting/`, `src/platform/reporting/report-standard/`).** The document PCAF Chapter 6 asks a bank to file is built from one content model in the checklist's order and rendered to PDF and Word by one renderer — and the renderer is now genuinely one: the visual theme, the block vocabulary and the two renderers moved from Part C's tree to `src/platform/reporting/report-standard/`, theme injected as a parameter, so Part C and Part A render through one engine without either domain importing the other. Part C's output is byte-identical and the golden test holds it. `GET /v1/pcaf/part-a/disclosure/:year` reads the register's reporting-year position and lays it out — coverage as assessed outstanding over the stated book (or absent, p.124), scope 3 a separate line, removals netted against nothing (p.126), the outstanding-weighted score with scope 3 apart (Box 6.1-6), the factor set named with its checksum, Annex 10.2 by sector, the improvement plan as the limitations section — and a year with no exposures is a **409**, never a document of zeros. `POST /v1/pcaf/part-a/exposures/:id/report` is the same document for one borrower, carrying its findings. Both are `read` and store nothing; both serve JSON, PDF or Word; every figure is one the register returned and nothing here recomputes. **The completed checklist can never reach a hundred per cent, and that is correct** — the inventory Chapter 6 ultimately asks for is the entity's *own* gross scope 1/2/3 (SLFRS S2 §29(a)), and this report is the §5.2 asset class, one input to it; the entity-inventory item is answered No with that reason, the same rule the GCF report follows. `tests/parta-report-golden.test.js` pins the document.

**The document a verifier can sign, and the one document a bank files (`src/domains/pcaf-part-a/reporting/{entity,identity,common-sections}.js`, `reporting/consolidated/`, `application/parta-consolidated.js`).** Read from the seat of an ISO 14064-3 / ISAE 3000 verifier, the arithmetic in the §5.2 and §5.9 documents was assurable and the apparatus around it was not: no organisational boundary or consolidation approach — and a checklist item claiming one that **always answered Yes** — no per-exposure audit trail, no responsible party, no uncertainty statement, no document identity, seven checklist items that could never fail, a cover reading *Re/insurer* on a bank's document, and no consolidated Part A disclosure at all. **The reporting entity is a recorded fact, not a query string**: the legal name, consolidation approach, boundary, fiscal year-end, GWP basis, preparer, approver and the classes not reported with reasons live on `parta_settings`, null by default, and every document prints each as stated or *Not stated* — a default printed in their place would be a claim the entity never made. The cover names the reporting entity, who prepared and approved, and the document's identity: a reference derived from a SHA-256 over the canonical facts, the build commit and the content hash, so one position rendered twice is one reference and a filed copy can be matched. The shared renderer takes the entity label from the model, so Part C's documents are byte-identical. Every Part A document opens with the entity and boundary, prints scope 1 and 2 apart, the option distribution, an intensity the roll-up computed, the fluctuation analysis, an uncertainty statement by score with everything absent listed once, an exposure register annex with one row per exposure resolving to its stored record, every factor row with the whole checksum, and where each SLFRS S2 line comes from. **No checklist item is a constant** — `tests/parta-report-golden.test.js` proves a book whose entity stated nothing answers No on five governance items — and INV-1 stays No by design. **The consolidated disclosure** (`GET /v1/pcaf/part-a/financed-emissions/:year`, `/disclosure`, `/register.csv`) lays every asset class side by side: the headline sums each recorded class on the boundary its section reports and names them, scope 3 apart; one data-quality score per class, never averaged across tables; coverage summed only across the classes in the book's currency, the excluded class named — which surfaced that the USD-denominated sovereign register had been dividing by an LKR book total; and every class not recorded is a row with a reason, the entity's overriding the system's. Sixteen conformance rules (A-DOC-01…16) prove it by execution.

**The §5.2 engine carries its own conformance matrix, and it is proved by execution (`src/domains/pcaf-part-a/domain/conformance.js`, `docs/PCAF-PART-A-CONFORMANCE.md`).** The north star's rule is that every module carries its own conformance evidence or it is not done, so §5.2 has a matrix in the shape Part C and the GCF pipeline already use: rule → the code that enforces it → the test that proves it, grouped from scope through classification, the option-to-score mapping, estimation, the six lines, the third verdict and its checks, the register, the disclosed score and the report. It is a single matrix — the report rules that shipped as a separate `reporting/conformance.js` are folded into it, because the two scopes never merge and this is one §5.2 statement, not two. `GET /v1/pcaf/part-a/conformance` serves it as cacheable reference data with the standing disclaimer that PCAF does not endorse software and the Third Edition additions have not been reviewed by the GHG Protocol; `tests/pcaf-parta-conformance.test.js` fails the build if a rule names a file or a test that does not resolve; and `scripts/conformance-evidence.js` now runs the §5.2 rules alongside Part C's and the GCF pipeline's, each rule's own test under coverage restricted to what it cites, so a citation that merely resolves is not mistaken for behaviour. One scope rule is proved by the absence of a path — the §5.2 engine has no import into Part C or GCF — and the rest by execution.

**Recalculation is a "shall", and the disclosure prints the entity's own protocol (`src/domains/pcaf-part-a/domain/recalculation.js`, `application/register.js`, migration `0010`).** Chapter 6 requires a stated base year, a significance threshold and the triggers that force a recalculation, and Part A holds them the way Part C does but on its own settings — the two scopes never merge, and the threshold sits beside a different weighting (outstanding amount, not premium) and triggers phrased for a lending book. A new org-wide `parta_settings` record holds `baseYear` (null by default, because a base year is a claim about history and belongs to the entity, so where none is set the disclosure says so rather than implying the current year), `significanceThresholdPct` (default 5, the GHG Protocol Scope 3 trigger), the default triggers and a free-text policy; `GET/PUT /v1/pcaf/part-a/settings` read and set it, and the disclosure's Recalculation section prints it, absent-when-unset. And `recompute()` now judges the movement against the entity's own threshold: a movement at or above it is a recalculation trigger that, once the lock-and-supersede lifecycle exists, requires a restatement with a recorded reason — today it is reported rather than silently absorbed, since nothing yet locks. The base year is org-wide rather than on each year's `parta_book` row on purpose: two years must not disagree about which year the base is.

**The roll-up reads a projection, and every path in it is a path into the record.** A stored exposure is several kilobytes, most of it the provenance trace, and the reporting-year position needs about twenty fields; `parta_exposure_rollup()` computes exactly those into a generated column at write time — 316 ms over 10,000 exposures against a one-second bound, the same defect `partc_assessments.rollup` was built to end. The paths are the record's own rather than a flattened shape of the column's, which is what lets the in-memory store project the same set with no column at all and is the only reason the suite can prove **on either store** that the projected roll-up equals the whole-record roll-up figure for figure. One trap is written down because it returns the opposite of the truth: `jsonb_strip_nulls` removes a null field and leaves the object that held it, so an exposure with no attribution factor projects as `attribution: {}`, which is truthy — `inflate()` normalises it and a test pins the count.

**The register writes, so it does not carry the engine's scope.** `POST /v1/pcaf/part-a/` was `read` because the engine stores nothing. The register's writes are `write`, and the two rules sit in that order in `scopes.js` because the first match wins — the broad stateless rule would otherwise have handed a read-only key the ability to record exposures. The engine routes are unchanged and still store nothing: the same request twice returns the same answer and issues no id.
...
**One seam, and everything goes through it (`src/platform/database/adapters/`).** There is one adapter per store — PostgreSQL, Firebase, Blobs, in-process — behind one interface, and `store.js` picks one *once* from the mode `capability()` resolved. It used to decide per verb, asking `isDurable()` ("is Firebase configured") rather than which store had been chosen, so `STORAGE_BACKEND=memory` on a Firebase deployment wrote to both. `tests/store-conformance.test.js` runs the same contract against every adapter the process can reach and states the differences that are real — only PostgreSQL is transactional; only PostgreSQL sorts numbers numerically — rather than leaving them to be found in production.

**Five record types used to be written past the seam, and a discarded write was reported as a success.** Lending projects, their monitoring entries, agent runs, supervisor pipeline runs and webhook subscriptions went straight to Firebase through `platform/bridge/firebase`, each writer opening `const db = getDatabase(); if (!db) return;`. On a deployment holding its records in PostgreSQL — which is every deployment — `POST /v1/projects` answered **201 Created** and kept nothing. They are on the seam now (migration `0005`), and the same call on a deployment that cannot persist is a **503 naming `DATABASE_URL`**. `tests/storage-seam.test.js` holds that and fails the build when a sixth record type reaches for the bridge; the bridge is 176 lines and does two things — the core engine read, and driving the seam's Firebase adapter.

**`put` is an upsert, and one caller needed an insert (`store.insert()`).** `POST /v1/desk/adopt` derives its id from the record — `inv_<recordId>` — so two adoptions of one pipeline candidate aimed at **the same primary key**. `put` resolves that with `ON CONFLICT … DO UPDATE`, so the second write updated the first row instead of colliding with it, the unique index on the origin never saw a second row to reject, and **both callers were told they had adopted it**. The second silently rewrote `origin`, the frozen screening verdict and the pledged mitigation — the three things this file says are written "once and never again", and the only reason a committee can be shown what it was actually told. It surfaced as a test that failed about one run in three, whose comment blamed database contention and loosened the assertion until it passed; the assertion that would have caught it is `ok.length === 1`. `store.insert()` refuses a record that is already there with the same `409 DUPLICATE` a unique index raises. Only PostgreSQL closes the race outright, in the primary key; the other three read and then write, which narrows the window without shutting it, and `tests/store-conformance.test.js` states that as a real difference rather than leaving it to be found.

**A caller that needs atomicity says so.** `store.transaction(fn, { name, required: true })` gives a real transaction on PostgreSQL, **refuses** on a durable store that has none (503 `NOT_TRANSACTIONAL`) because half of a lock-and-supersede on a real book is a position nobody can reconcile, and runs with a warning in-process, which is where a non-atomic run is acceptable. The in-process store also **refuses at its ceiling** (507 `STORE_FULL`) rather than dropping its oldest record — the same defect as "a book of 201 projects rolled up as 200 without a word", which this file already records having shipped once.
...
Note that under `auto`, **Blobs is never chosen**: the order is PostgreSQL, then Firebase, then memory, and a deployed site with none of them refuses writes. That is deliberate — the database belongs to the operator, and a site that quietly kept records inside Netlify would be holding a book nobody provisioned. Blobs remains selectable by `STORAGE_BACKEND=blobs`, and the reason string says which applied — but **it does not resolve on this deployment's function runtime**, proved on a healthy deploy with all three functions created and the setting in force, and `docs/ENVIRONMENTS.md` records it so nobody selects it again expecting a store.

**The function environment may not exceed 4 KB (`docs/DATA-LAYER.md`, `docs/ENVIRONMENTS.md`).** Netlify Functions are AWS Lambdas, and Lambda refuses to create a function whose configured environment — the operator's variables and the ones Netlify sets beside them — passes 4,096 bytes. The deploy fails at function creation, the message names a size and not a variable, and the site carries on serving the last build that worked: four merges went undeployed before the cause was found, because the symptom is *the change did not appear*. `FIREBASE_SERVICE_ACCOUNT` was most of the budget on a site holding nothing in Firebase; `DATABASE_URL` is about 120 bytes and buys the whole store. `config.validate()` estimates the environment on a serverless runtime and raises a **warning** at three quarters of the ceiling naming the largest variables — **names and byte counts only**, because that block reaches `/health`.

**A warning is never a refusal, and the first version of that check proved why.** It summed the whole of `process.env` inside the running function — the Lambda runtime's session token, paths and trace id included, none of which count against the ceiling — passed the warning line on an ordinary deployment, and the adapter treated the finding as a problem: every route on the live site answered `503 DEPLOYMENT_UNSAFE` for the life of that build, and every unit test was green while it did. `validate()` now returns **problems**, which a server refuses to start on and `netlify/functions/fintech-api.js` refuses every route but `/health` on, and **warnings**, which nothing refuses on; `environmentSize()` leaves the runtime's own variables out (`LAMBDA_RUNTIME_OWN`) and says in its own doc that it is an estimate. `tests/netlify-function.test.js` loads the real adapter under a Lambda-shaped environment and asserts a sign-in request is served, a crowded environment is a warning on `/health` and still served, and a real problem refuses — the refusal path had no test of its own either.
...
**The glossary found a fourth 1–5 scale.** Three were known and are documented as non-interchangeable: Part C's `(option) → score`, Part A's `(asset class, option) → score`, and GCF's four evidence tiers, which are deliberately words rather than numerals. Writing §1 turned up a fourth — `src/domains/lending/application/pcaf.js` defined its own 1–5 band under a header reading *"PCAF v3 Output Service"*, which is the likeliest source in this repository of a figure quoted as a PCAF score that is not one. The header says what it is now. **That is corrected, and it was worse than a name.** "PCAF v3" is gone from `src/` — there is no such standard to conform to; the edition is PCAF Part A, Third Edition (Dec 2025), implemented in another domain. But the memo templates did three things a bank would have acted on. A row read **"Bank's Financed Emissions"** for a figure that is attributed *embodied* carbon. A row labelled CarbonIQ's own factor-provenance band a **"PCAF Data Quality Score"** — the fourth 1–5 scale, printed under a PCAF heading, which is exactly the mistake `docs/GLOSSARY.md` §1 exists to prevent. And a row read **"Reporting Class | Scope 3 Category 15 (Financed Emissions)"**, which told a bank to book the embodied carbon of construction materials into the financed-emissions line of a regulated inventory. Every memo that states the figure now heads it *Attributed embodied carbon (A1–A3)* and carries, beside it, what it is not and where a Category 15 figure actually comes from. `tests/lending-pcaf-claim.test.js` sweeps for the removed rows — matching a memo **table row** rather than the words, because "PCAF Data Quality Score" also appears in these prompts meaning PCAF's own scale as something the bank reports, and whether *those* claims are right is a larger question about what the product says of itself rather than this correction.
...
- Config: `netlify.toml` — publishes `dist/ui`, the built frontend; the `staging` branch is a production-shaped context (`docs/ENVIRONMENTS.md`)
...
| `docs/PCAF-PART-A-BUSINESS-LOANS.md` | §5.2 business loans and unlisted equity: what is refused, what is merely recorded, the checks that validate a book's own data, and what is not built |
| `docs/PCAF-PART-A-RESEARCH.md` | **The reference guide for Part A**, and the document every section is built from: the ten asset classes, the consolidated data-quality matrix, Chapter 6, the supplement, the gaps the standard leaves, how banks disclose today, what is built, and the build order. §0 states how it is kept and §12 logs every change — a new finding is recorded there before the code that acts on it |
| `docs/PCAF-PART-A-CONFORMANCE.md` | The §5.2 conformance matrix, generated from `src/domains/pcaf-part-a/domain/conformance.js`: rule → implementation → proving test, per group |
