...
**Conformance (`services/gcf/conformance.js`).** 32 rules mapping ToR clause → implementation → proving test. `tests/gcf-conformance.test.js` fails the build when a cited file or a cited **test name** stops resolving — including a renamed test inside a file that still exists, which is exactly how a matrix goes quietly wrong. It caught a citation on the commit that introduced it, differing from the real test name by a curly apostrophe.
