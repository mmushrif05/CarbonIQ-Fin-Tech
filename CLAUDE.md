...
**The function environment may not exceed 4 KB (`docs/DATA-LAYER.md`, `docs/ENVIRONMENTS.md`).** Netlify Functions are AWS Lambdas, and Lambda refuses to create a function whose environment — names, values and the platform's own injected variables together — passes 4,096 bytes. The deploy fails at function creation, the message names a size and not a variable, and the site carries on serving the last build that worked: four merges went undeployed before the cause was found, because the symptom is *the change did not appear*. `FIREBASE_SERVICE_ACCOUNT` was most of the budget on a site holding nothing in Firebase; `DATABASE_URL` is about 120 bytes and buys the whole store. `config.validate()` measures the environment on a serverless runtime and raises a problem at three quarters of the ceiling naming the largest variables — **names and byte counts only**, because that block reaches `/health`.
