...
**The 4KB the whole deployment fits inside (`scripts/env-budget.js`, `scripts/firebase-env.js`):** AWS Lambda caps *all* environment variables for a function at 4,096 bytes combined, and exceeding it does not fail the build — it fails the **deploy**, at the upload step, after Initializing and Building have both gone green: *"Failed to create function: … environment variables exceed the 4KB limit"*. Nothing in the application is wrong, so nothing in the application reports it. Two things were spending that budget on nothing. Fifteen variables in `netlify.toml` set values identical to the defaults in `config/index.js`; and `FIREBASE_SERVICE_ACCOUNT` carried the whole service account as base64 — **3,149 bytes, 77% of the cap** — when `admin.credential.cert()` reads exactly three of its fields, and base64 inflates the secret by a third on top. Credentials are now read from `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL` and `FIREBASE_PRIVATE_KEY` (**1,875 bytes**), with the base64 blob still accepted so an existing deployment keeps working. `node scripts/env-budget.js` prints the budget by variable — names and byte counts, never values — and exits non-zero when a deploy would fail, so the ceiling is visible before Netlify finds it.
