...
**A failure now announces itself (`services/observability.js`):** every production defect this project has had arrived as "it isn't working", days late, with the cause reconstructed afterwards — the blank PDF, the agent answering `401 terminated`, the write refused with a 503. Errors are now reported to Sentry as they happen, with the request that caused them, and the event id is quoted back in the 500 response so a screenshot becomes a searchable incident. Three rules keep the reporting layer from becoming a liability of its own: it is **inert unless `SENTRY_DSN` is set** — no initialisation, no network calls, no behaviour change, so local development and the test suite are untouched; it **never sends a credential** — the API key, authorization and cookie headers are stripped and request bodies are never attached, because an insurer's book must not leak through the safety net; and it **never changes an outcome** — a reporting failure cannot turn a handled 4xx into a crash. Only what the application did not choose is reported: a validation error, an unknown key or a disabled feature is the system working, so 4xx and Joi failures are filtered out and 5xx are not.
